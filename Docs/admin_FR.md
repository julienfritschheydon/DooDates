...
*   **`handleDeleteUser(fingerprint: string)` :** Supprime toutes les entrées de `guest_quota_journal` *puis* l'entrée `guest_quotas` associée à un fingerprint. Ceci est crucial pour respecter les contraintes de clé étrangère. Requiert une confirmation (via `showDeleteConfirm`).
