...
### Architecture API - Mobile-First

- **Base URL :** `https://api.doodates.app` (prod) / `http://localhost:8083/api` (dev)
- **Authentication :** Bearer tokens (Supabase JWT) + refresh automatique
- **Format :** JSON optimisé mobile (payload minimal)
- **Versioning :** `/v1/` dans l'URL
- **Rate Limiting :** 2000 req/h mobile, 1000 req/h desktop, 100 req/h anonyme

### Standards Mobile-Optimized

- **HTTP Status Codes :** Standard REST
- **Error Format :** RFC 7807 (Problem Details) + mobile-friendly messages
- **Pagination :** Cursor-based optimisé pour scroll infini mobile
- **Filtering :** Query parameters légers pour connexions mobiles
- **CORS :** Configuré pour domaines autorisés + PWA
- **Offline Support :** Cache headers optimisés, sync queue
...
```json
{
  "emails": ["new-participant@example.com"],
