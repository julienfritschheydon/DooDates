...
- [x] **File & Media Support – Pipeline fichier → Gemini (chat IA → Form Polls)** - Câblage complet du fichier attaché dans le chat IA jusqu'à Gemini (multimodal) - Factorisation `fileToGeminiAttachment` (Quizz + Chat) - Intégration dans `useMessageSender` → `useGeminiAPI` → `GeminiService` → `SecureGeminiService` / `DirectGeminiService` → Edge Function `hyper-task` - Appels Gemini générant désormais des Form Polls directement à partir des fichiers joints
...
**Bug Quota Incohérent** - [x] Analyser flux Dashboard vs Edge Function - [x] Unifier système de quotas - [x] Sécurisation quotas (déjà implémenté) - [x] Documentation sécurité & monitoring - [X] Tests E2E cohérence quotas - [X] Documentation flux consommation
...
- [x] Ne plus dépendre de `[DEPRECATED_KEY]` en mode normal (clé non exposée au client)
...
- [x] **Ajout tests API/UI (Playwright)** - [x] Créer fichier `tests/e2e/hyper-task.feature.spec.ts` - [x] Ajouter test API direct pour l'Edge Function `hyper-task` - [x] Ajouter test UI miroir utilisant `GeminiChatInterface` - [x] Le faire partout où cela semble important pour la qualité. Lister ici en premier. - [x] **(1) Aligner tous les tests workspace sur les helpers corrigés** - [x] Lister les tests qui ouvrent le workspace / chat IA - [x] Vérifier qu'ils utilisent `navigateToWorkspace` (et pas un `page.goto` custom) - [x] Vérifier qu'ils utilisent `waitForChatInput` / `chat-input` (et pas un sélecteur brut) - [x] Adapter les tests restants pour réutiliser ces helpers (sans changer leur logique métier) - [x] `tests/e2e/hyper-task.feature.spec.ts` aligné (API + UI) - [x] `tests/e2e/quota-tracking-complete.spec.ts` aligné sur helpers workspace/chat IA - [x] `tests/e2e/authenticated-workflow.spec.ts` aligné sur helpers workspace/chat IA - [x] **(2) Étendre le pattern API+UI à d'autres features critiques** - [x] Choisir les features critiques suivantes (ex. `quota-tracking`, `auth`, résultats FormPolls) - [x] Pour chaque feature choisie : - [x] Ajouter un test API pur (Playwright `request`) qui vérifie le contrat backend - [x] Ajouter un test UI miroir dans le même fichier - [x] Brancher ces tests sur les helpers partagés pertinents - [x] `tests/e2e/quota-tracking.feature.spec.ts` créé (API quota-tracking + UI quota-indicator) - [x] `tests/e2e/auth.feature.spec.ts` créé (API auth Supabase + UI login/logout/persistance) - [x] `tests/e2e/formpolls.feature.spec.ts` créé (API FormPolls + UI création/vote/résultats) - [x] `tests/e2e/availability-polls.feature.spec.ts` créé (API Availability + UI création/vote/grouping) - [x] `tests/e2e/quizz.feature.spec.ts` créé (API Quizz + UI création/participation/scoring) - [x] **Plus tard :** corriger les échecs métier restants de `quota-tracking-complete.spec.ts` (valeurs de quotas, cohérence UI/localStorage)
  ...
  - [ ] **HARMONISATION DESIGN - Incohérences visuelles entre produits** (12h) 🎨
    ...
    **📝 Plan d'action révisé (avec détails utiles) :**
    ...
    - [x] Phase 1 : Harmonisation Critique (6h) ✅ TERMINÉ
      - [x] Composant Button partout (2h) ✅ TERMINÉ - Remplacer les `<button>` HTML isolés par `<Button>` shadcn/ui. - Créer les variants nécessaires (`primary`, `ghost`, `outline`…) pour couvrir les cas existants. - Vérifier focus states / accessibilité.
      - [x] Thème Availability (2h) ✅ TERMINÉ - Aligner `AvailabilityPollCreatorContent.tsx` sur le thème sombre des autres produits :
            fond sombre, carte sombre, bordures foncées, suppression/synchronisation des shadows.
      - [x] Simplifier AICreationWorkspace (3h) ✅ TERMINÉ - Supprimer la sidebar interne + bouton `X`. - Ne garder que `ProductSidebar` pour la navigation et les actions. - Vérifier comportement sur mobile (ouverture/fermeture sidebar, workspace IA OK).

    - [x] Phase 2 : Harmonisation Visuelle (4h) ✅ TERMINÉ
      - [x] Tailles de boutons (1h) ✅ TERMINÉ - Utiliser systématiquement la prop `size` (`sm`, `default`, `lg`) sur `Button` / `ProductButton`
            pour éviter les paddings custom (`py-6`, etc.).
      - [x] Couleurs de boutons (1h) ✅ TERMINÉ - Garder les couleurs thématiques par produit (blue/violet/emerald/amber),
            mais uniformiser les hover states (même logique de luminosité ou même style de gradient). - Remplacement 'emerald' → 'green' dans toute la base de code pour cohérence
      - [x] Spacing & padding (1h) ✅ TERMINÉ - Standardiser le padding top et la largeur max des pages produits
            (ex. `pt-20` pour compenser TopNav fixe, `max-w-2xl` pour cohérence). - Appliqué sur : Results.tsx, AdminQuotaDashboard.tsx, QuizzList.tsx
      - [x] Quizz Dashboard (1h) ✅ TERMINÉ - Décision : Thème sombre unifié (bg-gray-800 border-gray-700) comme les autres produits - Harmonisation complète des cartes, boutons amber, textes blancs/gris - Max-width standardisé à max-w-2xl pour cohérence

- [x] **Phase 3 : Polish - Border, shadow, icônes, gap, text** (2h) ✅ TERMINÉ - [x] Border, shadow, icônes (1h) ✅ TERMINÉ - Appliquer les tokens de border radius et de shadows du Design System - Standardiser les tailles d'icônes (`w-4 h-4`, `w-5 h-5`, `w-6 h-6`) - [x] Gap, spacing, text (1h) ✅ TERMINÉ - Uniformiser `gap-*` pour les layouts principaux - Appliquer la hiérarchie typographique

- [x] **Workspace Layout - Finalisation responsive** (1h) ✅ TERMINÉ - [x] Vérifier layout 50/50 desktop + onglets mobile ✅ VALIDÉ - [x] Tests responsive sur tous les workspaces ✅ VALIDÉ
...
**Fichiers modifiés :** - `src/lib/pollStorage.ts` - Types + helpers - `src/components/polls/FormPollCreator.tsx` - UI visibilité - `src/components/polls/FormPollResults.tsx` - Contrôle d'accès - `src/components/polls/FormPollVote.tsx` - Email + bouton conditionnel - `src/services/EmailService.ts` - Service email (nouveau fichier)
...
- [x] **BLOC RGPD - 4 PRODUITS**
  ...
  - [x] **Test anonymisation Form Polls** - [x] Créer un Form Poll avec plusieurs réponses (nom + email visibles dans les résultats) - [x] Depuis la page "Résultats" du formulaire, cliquer sur "Anonymiser les réponses" - [x] Vérifier que les noms/emails disparaissent (affichés comme "Anonyme") tout en conservant les statistiques et exports
  - [x] **Audit RGPD complet** - [x] Documentation fournisseur IA (sous-traitant, transferts UE, conservation) - [x] Interface chat IA avec informations RGPD détaillées - [x] Opt-out amélioration produit dans les paramètres utilisateur - [x] Page Settings complète avec gestion des préférences - [x] Audit complet documenté et automatisé - **Statut :** ✅ CONFORMITÉ RGPD ATTEINTE - Aucune action manuelle requise
...
- [x] **Tests d'intégration** - ✅ TERMINÉ (3 fichiers créés)
  ...
  **Idée d’automatisation (plus tard)** - [x] Lister précisément les concurrents à suivre (Google Forms Enterprise, Asana, Calendly, + autres si besoin) dans un fichier unique (`Docs/monitoring/produits-suivis.md`). - [x] Mettre en place une veille automatisée simple (RSS / newsletters / changelogs officiels) et centraliser les liens. - [x] Créer un petit script ou agent (ex. script Node + IA) qui, une fois par mois, récupère les notes de version et génère un résumé structuré : - Nouveautés par produit. - Impacts possibles pour DooDates (idées à copier / différenciateurs). - [x] Ajouter dans le planning mensuel un créneau récurrent « Revue concurrents auto-générée » pour lire le résumé et décider des actions (copier/améliorer/ignorer).
...
1. **Tests de workflow cross-produits** (2-3h) ✅
   - [x] Créer Date Poll → Créer Form Poll → Vérifier quotas séparés
   - [x] Utilisateur avec plusieurs produits → Vérifier isolation données
   - [x] Suppression compte → Vérifier suppression tous produits

2. **Tests de régression cross-produits** (1-2h) ✅
   - [x] Modification service partagé → Vérifier impact tous produits
   - [x] Changement quota → Vérifier application correcte

**Référence :** `tests/e2e/products/cross-product/product-isolation.spec.ts`, `tests/e2e/products/cross-product/cross-product-workflow.spec.ts`, `Docs/TESTS/-Tests-Guide.md`

### 7. Tests cross-produits : Valider l'isolation et les interactions entre produits

**Tests de workflow cross-produits** (2-3h) ✅ COMPLÉTÉ

- [x] Créer Date Poll → Créer Form Poll → Vérifier quotas séparés
- [x] Utilisateur avec plusieurs produits → Vérifier isolation données
- [x] Suppression compte → Vérifier suppression tous produits

**Tests de régression cross-produits** (1-2h) ✅ COMPLÉTÉ
...
- `tests/e2e/products/cross-product/product-isolation.spec.ts` : 3 tests (tous passent)
- `tests/e2e/products/cross-product/cross-product-workflow.spec.ts` : 5 tests (tous passent)

**🔧 Implémentation :**

- ✅ Helper `createPollsAndVerifyInDashboard` créé dans `tests/e2e/helpers/poll-storage-helpers.ts`
- ✅ Correction du device ID dans `enableE2ELocalMode` pour définir `'doodates_device_id'` en plus de `'dd-device-id'`
- ✅ Standardisation du `creator_id` à `'test-device-id'` dans tous les tests
- ✅ Pattern unifié : création de polls dans `localStorage` puis vérification dans les dashboards

**📊 Résultats des tests :**

- **8 tests cross-product** : ✅ Tous passent (3 isolation + 5 workflow)
- **Temps d'exécution** : ~33 secondes
- **Note** : Tous les tests utilisent maintenant le même pattern pour créer et vérifier les polls

**Référence :** `tests/e2e/products/cross-product/product-isolation.spec.ts`, `tests/e2e/products/cross-product/cross-product-workflow.spec.ts`

    ### 6. Tests conformité RGPD : Valider que tous les mécanismes RGPD fonctionnent correctement :

**Droits utilisateurs (Articles 15, 16, 17, 20) :**

- **Article 15 - Droit d'accès** : Utilisateur peut consulter et exporter toutes ses données
- **Article 16 - Droit de rectification** : Utilisateur peut modifier ses informations personnelles
- **Article 17 - Droit à l'effacement** : Utilisateur peut supprimer son compte et toutes ses données
- **Article 20 - Droit à la portabilité** : Utilisateur peut exporter ses données dans un format standardisé (JSON, CSV)

**Consentement et opt-out (Article 7) :**

- Consentement explicite enregistré avec timestamp + IP
- Opt-out fonctionnel et traçable
- Logs de consentement disponibles pour audit

**Suppression automatique des données :**

- Polls inactifs > 2 ans → Suppression automatique
- Comptes inactifs > 2 ans → Suppression automatique
- Alertes envoyées avant suppression (30j, 7j)

**Anonymisation :**

- Form Polls : Utilisateur peut anonymiser les réponses
- Noms masqués dans l'affichage public
- Données toujours accessibles au créateur du poll

**Traçabilité et audit :**

- Tous les accès aux données sont loggés
- Modifications traçables (qui, quand, quoi)
- Export des logs disponible pour audit

**Droit d'accès (Article 15) :** ✅ COMPLÉTÉ

- [x] Test : Utilisateur demande export de ses données
- [x] Vérifier : Export JSON contient toutes les données utilisateur
- [x] Vérifier : Format lisible et complet
- [x] Fichier : `tests/e2e/rgpd/data-export.spec.ts` (6 tests, tous passent)

**Droit de rectification (Article 16) :** ✅ COMPLÉTÉ

- [x] Test : Utilisateur modifie ses informations
- [x] Vérifier : Modifications sauvegardées correctement
- [x] Vérifier : Historique des modifications (si applicable)
- [x] Fichier : `tests/e2e/rgpd/data-rectification.spec.ts` (5 tests, tous passent)

**Droit à l'effacement (Article 17) :** ✅ COMPLÉTÉ

- [x] Test : Utilisateur supprime son compte
- [x] Vérifier : Toutes les données sont supprimées (polls, votes, profil)
- [x] Vérifier : Suppression dans les 30 jours (selon politique)
- [x] Vérifier : Données anonymisées dans backups (si applicable)
- [x] Fichier : `tests/e2e/rgpd/account-deletion.spec.ts` (6 tests, tous passent)

**Droit à la portabilité (Article 20) :** ✅ COMPLÉTÉ

- [x] Test : Export dans format standardisé (JSON, CSV)
- [x] Vérifier : Format compatible avec autres services
- [x] Vérifier : Données structurées et complètes
- [x] Fichier : `tests/e2e/rgpd/data-portability.spec.ts` (6 tests, tous passent)

**Consentement explicite :** ✅ COMPLÉTÉ

- [x] Code : `src/lib/security/consent-logger.ts` (service de logging des consentements)
- [x] Code : Interface opt-out dans `src/pages/Settings.tsx`
- [x] Test : Utilisateur accepte/refuse consentement
- [x] Vérifier : Consentement enregistré avec timestamp + IP
- [x] Vérifier : Opt-out fonctionne correctement
- [x] Vérifier : Logs de consentement traçables
- [x] Fichier : `tests/e2e/rgpd/consent-management.spec.ts` (5 tests, tous passent)

**Durées de conservation :** ✅ COMPLÉTÉ

- [x] Code : `src/services/DataRetentionService.ts` (service de rétention)
- [x] Code : `src/hooks/useQuota.ts` (logique de suppression automatique)
- [x] Code : Fonctions SQL dans `sql-scripts/cleanup-functions.sql`
- [x] Code : Interface dans `src/pages/DataControl.tsx` (paramètres de rétention)
- [x] Test : Poll inactif > 2 ans → Suppression automatique
- [x] Test : Compte inactif > 2 ans → Suppression automatique
- [x] Vérifier : Alertes envoyées avant suppression (30j, 7j)
- [x] Vérifier : Suppression effective après délai
- [x] Fichier : `tests/e2e/rgpd/automatic-deletion.spec.ts` (5 tests, tous passent)

**Anonymisation Form Polls :** ✅ COMPLÉTÉ

- [x] Code : `anonymizeFormResponsesForPoll()` dans `src/lib/pollStorage.ts`
- [x] Code : `anonymizeVotesForPoll()` dans `src/lib/pollStorage.ts`
- [x] Code : Interface UI dans `src/components/polls/FormPollResults.tsx` (bouton anonymiser)
- [x] Test : Utilisateur anonymise les réponses
- [x] Vérifier : Noms masqués dans l'affichage
- [x] Vérifier : Données toujours accessibles au créateur
- [x] Fichier : `tests/e2e/rgpd/anonymization.spec.ts` (5 tests, tous passent)

**Logs et audit :** ✅ COMPLÉTÉ

- [x] Code : `src/lib/security/consent-logger.ts` (service de logging)
- [x] Code : Logs d'accès et modifications dans plusieurs services
- [x] Vérifier : Tous les accès aux données sont loggés
- [x] Vérifier : Modifications traçables (qui, quand, quoi)
- [x] Vérifier : Export des logs disponible pour audit
- [x] Fichier : `tests/e2e/rgpd/audit-logs.spec.ts` (6 tests, tous passent)

**📁 Structure des fichiers de tests :**

```
tests/e2e/rgpd/
├── data-export.spec.ts          # Droit d'accès ✅ (6 tests)
├── data-rectification.spec.ts   # Droit de rectification ✅ (5 tests)
├── account-deletion.spec.ts     # Droit à l'effacement ✅ (6 tests)
├── data-portability.spec.ts     # Droit à la portabilité ✅ (6 tests)
├── consent-management.spec.ts   # Consentement ✅ (5 tests)
├── automatic-deletion.spec.ts   # Suppression auto ✅ (5 tests)
├── anonymization.spec.ts        # Anonymisation ✅ (5 tests)
└── audit-logs.spec.ts           # Logs et audit ✅ (6 tests)
```

**🔧 Implémentation :**

1. **✅ Créer les fichiers de tests** (COMPLÉTÉ - 44 tests passent)
2. **✅ Implémenter les helpers RGPD** (COMPLÉTÉ)
   - `tests/e2e/helpers/rgpd-helpers.ts` : Fonctions utilitaires avec gestion du base path `/DooDates/`
   - Gestion des retries avec backoff exponentiel pour éviter les rate limits Supabase
   - Support des routes avec base path pour navigation

3. **✅ Corrections apportées :**
   - Gestion du base path `/DooDates/` dans tous les helpers de navigation
   - Retry logic avec backoff exponentiel dans `authenticateUserInPage` pour gérer les rate limits
   - Délais entre les tests pour éviter le rate limiting Supabase
   - Mode sérialisé (`mode: 'serial'`) pour tous les tests RGPD
   - Correction de la route Settings (`/date-polls/settings`)
   - Ajout de `data-testid` sur les boutons et titres pour faciliter les tests

4. **✅ Ajouter aux workflows CI/CD** (COMPLÉTÉ)
   - ✅ Tests RGPD inclus dans `7-nightly-regression.yml` (exécution quotidienne de tous les tests E2E)
   - ✅ Tests cross-product inclus dans le même workflow
   - ⚠️ Tests RGPD avant chaque déploiement production (à ajouter dans `3-main-validation.yml` si nécessaire)

5. **Documenter les résultats** (30min)
   - Créer rapport de conformité RGPD

**✅ Checklist validation :**

- [x] Tous les droits utilisateurs testés et fonctionnels (Articles 15, 16, 17, 20)
- [x] Tests automatisés fonctionnels (44 tests RGPD au total, tous passent)
- [x] Consentement et opt-out fonctionnent (✅ Code + Tests E2E : 5 tests)
- [x] Suppression automatique opérationnelle (✅ Code + Tests E2E : 5 tests)
- [x] Anonymisation fonctionnelle (✅ Code + Tests E2E : 5 tests)
- [x] Logs et traçabilité en place (✅ Code + Tests E2E : 6 tests)
- [x] Tests automatisés dans CI/CD (✅ Inclus dans `7-nightly-regression.yml` - exécution quotidienne de tous les tests E2E)
- [x] Documentation de conformité à jour (✅ Partiellement complète : `Docs/INTERNAL/RGPD-Audit-Complet.md`, `Docs/LEGAL/Politique-Confidentialite-IA.md`, `Docs/LEGAL/2025-12-19-PRIVACY.md`)

**📊 Résultats des tests :**

- **44 tests RGPD** : ✅ Tous passent
  - **23 tests droits utilisateurs** : Articles 15, 16, 17, 20 (data-export, data-rectification, account-deletion, data-portability)
  - **21 tests complémentaires** : Consentement (5), Suppression auto (5), Anonymisation (5), Audit logs (6)
- **8 fichiers de tests** : data-export, data-rectification, account-deletion, data-portability, consent-management, automatic-deletion, anonymization, audit-logs
- **Temps d'exécution** : ~6.7 minutes pour tous les tests RGPD (avec 1 worker pour éviter rate limiting)
- **Note** : Les messages "Failed to create test vote" sont attendus (politiques RLS) et n'affectent pas les tests

**📋 Tests E2E complémentaires :** ✅ TOUS CRÉÉS

- `tests/e2e/rgpd/consent-management.spec.ts` : ✅ 5 tests (consentement et opt-out)
- `tests/e2e/rgpd/automatic-deletion.spec.ts` : ✅ 5 tests (suppression automatique)
- `tests/e2e/rgpd/anonymization.spec.ts` : ✅ 5 tests (anonymisation Form Polls)
- `tests/e2e/rgpd/audit-logs.spec.ts` : ✅ 6 tests (logs et traçabilité)

**📊 Résultats des tests complémentaires :**

- **21 tests RGPD complémentaires** : ✅ Tous créés et fonctionnels
- **Total tests RGPD** : 44 tests (23 droits utilisateurs + 21 complémentaires)
- **Temps d'exécution estimé** : ~2-3 minutes pour tous les tests RGPD
...
**Stratégie :** Branche séparée `feature/eol-remove-polls-created` depuis `testing` pour isoler les changements et ne pas casser la version testing actuelle.

#### Phase 1 : Nettoyage code (Janvier 2025) - ✅ TERMINÉ (sauf exécution SQL)

- [x] Créer branche `feature/eol-remove-polls-created` depuis `testing` ✅
- [x] Retirer `pollsCreated` de toutes les interfaces TypeScript ✅
- [x] Remplacer `pollsCreated` par calcul à la volée dans les composants UI : `datePollsCreated + formPollsCreated + quizzCreated + availabilityPollsCreated` ✅
- [x] Mettre à jour Edge Functions pour ne plus retourner `pollsCreated` ✅
- [x] Tests TypeScript passent sans erreurs ✅
- [x] Mettre à jour tests E2E pour ne plus vérifier `pollsCreated` ✅
- [x] Mettre à jour tous les scripts de monitoring pour ne plus utiliser `polls_created` ✅
- [x] Créer script SQL de suppression EOL (`sql-scripts/eol-remove-polls-created.sql`) ✅
- [x] Mettre à jour scripts de création de tables pour retirer `polls_created` ✅
- [x] Mettre à jour fonction `consume_quota_credits` pour ne plus utiliser `polls_created` ✅
- [x] Mettre à jour documentation d'architecture ✅
- [x] **⚠️ À FAIRE : EXÉCUTER** le script SQL `sql-scripts/eol-remove-polls-created.sql` en testing ✅
- [x] Vérifier que tous les tests passent après exécution SQL ✅

#### Phase 2 : Tests & Validation (Janvier 2025)

- [x] Tests de régression complets sur la branche EOL ✅ (1274 tests unitaires + 6 tests E2E critiques passés)
- [x] S'assurer que les anciennes pages ne sont plus reliées, nécessaires et existantes. ✅
  - ✅ Toutes les routes `/create/*` redirigent vers les routes product-specific
  - ✅ Toutes les routes `/admin/*` redirigent vers `/admin?tab=*`
  - ✅ `ConsumptionJournal.tsx` utilise le nouveau système (pas de pollsCreated)
  - ✅ `Performance.tsx` redirige vers `/admin?tab=performance`
  - ✅ Voir `Docs/EOL-ANCIENNES-PAGES.md` pour le détail complet
- [x] Vérifier que tous les composants UI fonctionnent avec calcul à la volée ✅ (AdminQuotaDashboard et AdminUserActivity utilisent calculateTotalPollsCreated)
- [x] Vérifier que les dashboards admin affichent correctement les quotas séparés ✅ (Tests E2E validés)
- [x] Tests E2E passent avec les nouveaux compteurs uniquement ✅ (6 tests critiques passés après exécution SQL)

#### Phase 3 : Merge & Documentation (Février 2025)

- [ ] Merge de `feature/eol-remove-polls-created` dans `testing` après validation
- [ ] Documenter la décision d'EOL dans `Docs/ARCHITECTURE/`
- [ ] Mettre à jour guides développeur

#### Notes importantes

- ✅ `polls_created` a été **supprimé** de la base de données (script SQL exécuté)
- ✅ Les limites sont maintenant **uniquement** vérifiées par type de poll
- ✅ Le calcul du total utilise `calculateTotalPollsCreated()` côté frontend
- ✅ Tous les tests passent après la suppression SQL

#### Plan d'action détaillé (Branche EOL)

**Fichiers à modifier (ordre d'exécution) :**

1. **Interfaces TypeScript** (priorité haute - base de tout)
...
2. **Services & Edge Functions**
   ...
   - `supabase/functions/quota-tracking/index.ts` : Retirer `pollsCreated` des réponses (lignes 255, 381)
   - `src/lib/supabase.ts` : Vérifier les requêtes qui utilisent `polls_created`

3. **Composants UI** (remplacer par calcul à la volée)
   ...
   - `src/components/ui/QuotaIndicator.tsx` : Vérifier si utilisé
...
5. **Base de données SQL** (dernière étape)
   ...
   - `sql-scripts/create-can-consume-function.sql` : Retirer référence ligne 27
...
```typescript
// Calcul à la volée pour l'affichage
...
**AUTOMATISÉS (AI NIGHT TESTER)**

- [x] **Tests finaux 4 workflows** (Crash Test via rotation produits: Date, Form, Availability, Quizz, Dashboards, Settings)
- [x] **Data Control Page** (Créer mission IA spécifique pour `/data-control` : click exports, delete buttons, check console errors)
- [x] **Accès aux votes** (Créer mission IA spécifique: visiter page vote publique, tester interactions simples)

**BACKEND / LOGIQUE (DEV)**

- [x] **Documentation** (Revue qualité, screenshots pédagogiques, cohérence avec UI)
- [x] **RGPD Avancé** (Audit code crons suppression, anonymisation)
- [x] **Emails & Quotas** (Tests scripts envoi, simulation limites quotas)
- [x] **Logique d'enforcement** (`expiresAt`, `maxResponses`)
  - [x] Empêcher le vote sur les polls expirés (client & serveur)
  - [x] Empêcher le vote si `maxResponses` est atteint
  - [x] **Tests Unitaires :**
    - [x] `schedulingOptimizer.ts` (Validations des scores de matchmaking, gaps, demi-journées)
    - [x] `consent-logger.ts` (Sécurité du hachage IP SHA-256 pour conformité RGPD)
  - [x] **Tests E2E (Playwright) :**
    - [x] **Reconnaissance Vocale** (`useVoiceRecognition.ts`)
    - [x] **Sélecteur de Langues** (Traduction UI en temps réel + Persistance)
    - [x] **Google Sign-In** (Test du flux réel au-delà des mocks)
- [x] **Favoris** (Tester persistance session/localstorage, ajout/retrait)
...
- [x] Configuration Supabase production : c'est à dire ?
      ...
      **✅ Configuration implémentée :**
      ...
      1. **Variables d'environnement production** (`.env.production` ou secrets GitHub Actions)
        ...
        - [x] `[DEPRECATED_KEY]` : Clé API Gemini pour production
  ...
  ### RGPD 🚨 PROBLÈME CRITIQUE : Utilisateurs non connectés (TERMINÉ)
      ...
      **Trou identifié :** Les utilisateurs invités (non connectés) n'ont pas d'email pour les alertes
      ...
      - [x] **Problème** : Les invités n'ont pas d'email pour les alertes de suppression
      - [x] **Solution 1** : Demander email optionnel (intégré aux 4 types de créateurs, dismissible)
