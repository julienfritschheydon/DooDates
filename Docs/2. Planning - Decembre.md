...
### 📌 LUNDI 9 DÉCEMBRE - JOUR 6
  ...
  - [ ] **Quizz : Aligner avec les autres produits ou garder séparé ?**
    - Actuellement : Pricing/Docs SANS sidebar (pages marketing)
    - Date/Form/Availability : Pricing/Docs AVEC sidebar
    - Question : Faut-il mettre Quizz avec le même pattern que les autres ?

  - [x] Les menus de gauches doivents se fermer automatiquements et être facile à ouvrirs

  - [ ] **Navigation calendrier vers le passé :** (reporté)
...
- [ ] **📊 MOYENNE - Bug Quota Incohérent** (4-6h) - Dashboard vs Edge Function
  ...
  - [ ] **Faire comparer les 4 expériences avec l'ancienne et noter toutes les différences: naviguer partout, et essayer l'expérience totale**

  - [ ] **Tester à la main aussi les 4 produits (date/form/availability/quiz)**
        - [ ] favories à tester
          - [ ] Verification
...
🔹 **HARMONISATION DESIGN - Incohérences visuelles entre produits** (12h) 🎨
  ...
  **📋 Analyses complètes :**
  - `Docs/ARCHITECTURE/2025-12-08-ANALYSE-DIFFERENCES-VISUELLES-PRODUITS.md`
  - `Docs/ARCHITECTURE/2025-12-08-ANALYSE-COMPLETE-INCOHERENCES.md` ⭐ **NOUVEAU**
  
  **🔍 10 Incohérences identifiées :**
  
  **🔥 PRIORITÉ HAUTE (6h) - Impact UX majeur**
  
  1. **Bouton Retour** (1h) ⚠️ **NOUVEAU**
     - ✅ Quizz : Bouton `<ArrowLeft />` dans `QuizzCreate.tsx` ligne 251
     - ❌ Date/Form/Availability : Aucun bouton retour
     - **Impact :** Navigation incohérente entre produits
     - **Décision :** Ajouter partout ou supprimer de Quizz ?
  
  2. **Composant Button** (2h) ⚠️ **NOUVEAU**
     - Availability : Utilise `<Button>` shadcn/ui
     - Quizz : Utilise `<button>` HTML natif
     - Date/Form : Mix des deux
     - **Impact :** Styles, accessibilité, maintenance
     - **Solution :** Utiliser `<Button>` shadcn/ui partout
  
  3. **Availability - Thème clair** (2h) 🔥
     - Fond gris clair (`bg-gray-50`) au lieu de noir
     - Carte blanche au lieu de sombre
     - **Fichier :** `src/pages/AvailabilityPollCreatorContent.tsx`
  
  4. **AICreationWorkspace - Double sidebar** (3h) ⚠️
     - Sidebar interne avec bouton X
     - Redondance avec `ProductSidebar`
     - **Fichier :** `src/components/prototype/AICreationWorkspace.tsx`
  
  **⚠️ PRIORITÉ MOYENNE (4h) - Impact visuel**
  
  5. **Tailles de Boutons** (1h) ⚠️ **NOUVEAU**
     - Quizz Landing : `py-6` (24px padding)
     - Availability : `py-2` (8px padding)
     - **Solution :** Standardiser avec `size` prop
  
  6. **Couleurs de Boutons** (1h) ⚠️ **NOUVEAU**
     - Quizz : Gradients `from-amber-500 to-yellow-500`
     - Availability : Solide `bg-emerald-600`
     - **Solution :** Uniformiser hover states
  
  7. **Spacing & Padding** (1h) ⚠️ **NOUVEAU**
     - Availability : `pt-20` (80px padding top)
     - Quizz : Aucun padding top
     - **Solution :** Standardiser à `pt-8` ou `pt-12`
  
  8. **Quizz Dashboard - Design différent** (1h) 📊
     - Statistiques globales uniquement pour Quizz
     - Design de cartes différent
     - **Fichier :** `src/components/products/ProductDashboard.tsx`
  
  **📝 PRIORITÉ BASSE (2h) - Polish**
  
  9. **Border Radius, Shadows, Icônes** (1h) ⚠️ **NOUVEAU**
     - `rounded-lg` vs `rounded-xl`
     - `shadow-sm` vs `shadow-lg` vs aucune
     - Icônes `w-4 h-4` vs `w-5 h-5`
  
  10. **Gap, Spacing, Text Sizes** (1h) ⚠️ **NOUVEAU**
     - `gap-2` vs `gap-3` vs `gap-4`
     - `text-sm` vs `text-base` vs `text-lg`
  
  **📝 Plan d'action révisé :**
  - [ ] Phase 1 : Harmonisation Critique (6h)
    - [ ] Bouton retour partout (1h)
    - [ ] Composant Button partout (2h)
    - [ ] Thème Availability (2h)
    - [ ] Simplifier AICreationWorkspace (3h)
  - [ ] Phase 2 : Harmonisation Visuelle (4h)
    - [ ] Tailles & couleurs boutons (2h)
    - [ ] Spacing & padding (1h)
    - [ ] Quizz Dashboard (1h)
  - [ ] Phase 3 : Polish (2h)
  ...
  **🎯 Recommandations :**
  - [ ] Créer Design System (`src/config/design-tokens.ts`)
  - [ ] Créer composants génériques (`ProductButton`, `ProductCard`, `ProductHeader`)
  - [ ] Documenter guide de style (`Docs/DESIGN-SYSTEM.md`)
...
### 📌 VENDREDI 13 DÉCEMBRE - JOUR 10
  ...
  - [x] ~~Préparation refactor gemini.ts~~ → **TERMINÉ** (06/12) - Simplifié en mode direct
...
### 5. Définir limites exactes par type de poll
**Fichier concerné :**
...
**À faire dans le planning de décembre :**
