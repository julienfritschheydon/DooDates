...
- [x] **Tests Gemini - SIMPLIFICATION MODE DIRECT** ✅ **TERMINÉ**
  ...
  **Tests finaux :** 59 tests, tous passent (après ajustements)
  ...
  - [ ] **Prototype Aide aux Devoirs** (2h + 1h) 🎯 **RÉUTILISATION MAXIMALE**
    
    **Principe :** Réutiliser le code Form Polls existant, créer le minimum
    
    **Phase 1 : Spécifications** (30 min)
    - [ ] Définir le flux utilisateur (parent/enfant)
    - [ ] Identifier les différences avec Form Polls :
      - Photo → extraction questions (Gemini Vision)
      - Correction automatique avec feedback
      - Score + encouragements
    - [ ] Lister les composants réutilisables vs à créer
    
    **Phase 2 : Prototype** (1h30)
    - [ ] Réutiliser `FormPollService` pour la structure questions/réponses
    - [ ] Ajouter prompt Gemini Vision (extraction photo → questions)
    - [ ] Adapter `FormVotePage` → `QuizzPage` (copie minimale)
    - [ ] Ajouter logique scoring (simple: réponse correcte/incorrecte)
    
    **À NE PAS CRÉER :**
    - ❌ Nouveau système de stockage (réutiliser form_polls)
    - ❌ Nouvelle UI complète (adapter l'existante)
    - ❌ Nouveau backend (même Edge Functions)
