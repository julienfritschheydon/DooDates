...
test.describe("Console Errors & React Warnings", () => {
  test.skip(
    ...
    "Console error checks optimized for Chrome",
  );
  ...
  test("devrait ne pas avoir d'erreurs console sur la page d'accueil @smoke", async ({
    page,
    browserName,
  }) => {
    ...
    await page.goto("/date/workspace/date?e2e-test=true", { waitUntil: "domcontentloaded" });
    ...
    const filteredErrors = consoleErrors.filter((error) => {
      return (
        ...
        !error.includes("VITE_GEMINI_API_KEY manquante") && // Ignorer le warning de clé manquante en test
  ...
  test("devrait ne pas avoir de warnings React Hooks @critical", async ({ page, browserName }) => {
    ...
    await page.goto("/form/workspace/form?e2e-test=true", { waitUntil: "domcontentloaded" });
    await waitForNetworkIdle(page, { browserName });
    await waitForReactStable(page, { browserName });
    ...
    try {
      ...
      if (tagName === "input" || tagName === "textarea") {
        ...
        actualInput = page
          .locator(
            'input[placeholder*="message"], textarea[placeholder*="message"], input[type="text"], textarea',
          )
          .first();
        ...
        .waitForFunction(
          () => {
            const messages = Array.from(
              document.querySelectorAll('[class*="message"], [class*="Message"]'),
            );
    ...
    const hasNetworkError =
      pageContent.includes("Problème de connexion") ||
    ...
    if (browserName === "webkit" && hasNetworkError) {
      console.log(
        "⚠️ Webkit: Mock n'a pas intercepté les requêtes. Le code fonctionne en production.",
      );
      console.log(
        "⚠️ Skip du test sur webkit - problème connu avec les routes d'interception Playwright",
      );
      test.skip();
    ...
    if (hasError) {
      ...
      await page.screenshot({
        path: "test-results/debug-console-errors-ia-error.png",
        fullPage: true,
      });
      ...
      console.error(
        "💡 Solution: Configurer l'Edge Function ou utiliser un mock pour les tests E2E",
      );
      throw new Error(
    ...
    try {
      await waitForElementReady(page, '[data-testid="create-form-button"]', {
        browserName,
      ...
      const hasPollSuggestion =
        pageContent.includes("pollSuggestion") || pageContent.includes("poll-suggestion");
