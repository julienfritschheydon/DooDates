...
test.describe("Guest Quota System", () => {
  ...
  test("Quota Supabase créé automatiquement pour nouveau guest", async ({ page, browserName }) => {
    ...
    await page.goto("/DooDates/create/ai?type=date&e2e-test=false", {
  ...
  test("Consommation crédits message IA bloque si limite atteinte", async ({
    page,
    browserName,
  }) => {
    const timeouts = getTimeouts(browserName);
  ...
  test("Migration localStorage vers Supabase (données existantes)", async ({
    page,
    context,
    browserName,
  }) => {
