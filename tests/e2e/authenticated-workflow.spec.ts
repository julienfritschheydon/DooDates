...
test.describe("Authenticated User Workflow", () => {
  ...
  test("should allow user to sign up and access premium features", async ({
    page,
    browserName,
  }) => {
  ...
  test("should allow authenticated users to create multiple conversations", async ({
    page,
    browserName,
  }) => {
    const timeouts = getTimeouts(browserName);
  ...
  test("should migrate guest data when user authenticates", async ({ page, browserName }) => {
    ...
    const guestData = await page.evaluate(() => {
      return Object.keys(localStorage).filter(
        (k) => k.includes("conversation") || k.includes("gemini"),
      );
    });
  ...
  test("should access premium features when authenticated", async ({ page, browserName }) => {
    ...
    await expect(newPage.locator('[data-testid="chat-input"]')).toBeVisible({
      timeout: timeouts.element,
    });
  ...
  test("should handle sign out and return to guest mode", async ({ page, browserName }) => {
    ...
    await waitForReactStable(page, { browserName });
    const messageVisible = await safeIsVisible(
      page.locator("text=Authenticated conversation before signout"),
    );
    if (!messageVisible) {
    ...
    const stillVisible = await safeIsVisible(
      page.locator("text=Authenticated conversation before signout"),
    );
    if (stillVisible) {
  ...
  test("should show correct quota progression for authenticated users", async ({
    page,
    browserName,
  }) => {
    const timeouts = getTimeouts(browserName);
