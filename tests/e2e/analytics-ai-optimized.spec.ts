...
import {
  setupAllMocks,
  setupAllMocksWithoutNavigation,
  setupAllMocksContext,
} from "./global-setup";
...
test.describe.skip("Analytics IA - Suite Optimisée", () => {
  ...
  test.skip(
    ...
    "Routes interception non fonctionnelle sur WebKit/Safari",
  );
  ...
  test("1. Setup: Créer et clôturer un FormPoll avec 3 réponses (optimisé) @smoke @critical", async ({
    ...
    const analyticsPanel = await waitForElementReady(page, "[data-testid='analytics-panel']", {
      browserName,
    ...
    await waitForReactStable(page, { browserName });
    const insightCard = await waitForElementReady(page, "[data-testid='insight-card']", {
      browserName,
      timeout: timeouts.element,
    }).catch(() => null);

    const insightsAccordion = await waitForElementReady(page, "text=/.*Insights automatiques.*/", {
      browserName,
      timeout: timeouts.element,
    });
  ...
  test("2. Quick Queries et Query Personnalisée (combiné) @functional", async ({
    page,
    browserName,
  }) => {
    const timeouts = getTimeouts(browserName);
    ...
    if (hasQuickQuery) {
      ...
      await waitForReactStable(page, { browserName });
      const quickResponse = await waitForElementReady(page, "[data-testid='analytics-response']", {
        browserName,
    ...
    const queryInput = page
      .locator("[data-testid='analytics-query-input'], textarea[placeholder*='question']")
      .first();
    ...
    if (hasQueryInput) {
      ...
      await waitForReactStable(page, { browserName });
      const customResponse = await waitForElementReady(page, "[data-testid='analytics-response']", {
        browserName,
        timeout: timeouts.element * 1.5,
      }).catch(() => null);
      if (customResponse) {
  ...
  test("3. Quotas et Cache (combiné) @functional", async ({ page, browserName }) => {
    ...
    if (hasQueryInput) {
      ...
      await waitForReactStable(page, { browserName });
      await waitForElementReady(page, "[data-testid='analytics-response']", {
        browserName,
        timeout: timeouts.element * 1.5,
      }).catch(() => {});
      ...
      const quotaLocator = page
        .locator("[data-testid='quota-indicator'], [data-testid='quota-info'], text=/quota/i")
        .first();
