...
test.describe("🔒 RGPD - Suppression automatique", () => {
  ...
  test("RGPD-AUTO-DELETE-01: Utilisateur peut configurer les durées de conservation", async ({
    page,
  }) => {
    ...
    expect(pageContent).toMatch(
      /données|data|conservation|retention|30 jours|12 mois|conversation|sondage/i,
    );
  ...
  test("RGPD-AUTO-DELETE-02: Utilisateur peut activer/désactiver suppression automatique", async ({
    page,
  }) => {
    // Create and authenticate test user
    const { data: signUpData } = await createTestUser(testEmail, testPassword);
  ...
  test("RGPD-AUTO-DELETE-04: Utilisateur peut configurer les notifications email", async ({
    page,
  }) => {
    // Create and authenticate test user
    const { data: signUpData } = await createTestUser(testEmail, testPassword);
