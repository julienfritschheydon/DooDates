...
test.describe('🔒 RGPD - Consentement et opt-out (Article 7)', () => {
  ...
  test('RGPD-CONSENT-01: Utilisateur peut accéder aux paramètres de consentement', async ({ page }) => {
  ...
  test('RGPD-CONSENT-02: Utilisateur peut activer/désactiver consentement amélioration produit', async ({ page }) => {
    // Create and authenticate test user
    const { data: signUpData } = await createTestUser(testEmail, testPassword);
    userId = signUpData?.user?.id || null;
    ...
    const consentToggle = page.locator('input[type="checkbox"]').filter({ 
      hasText: /amélioration|improvement/i 
    }).or(
      page.locator('text=/amélioration|improvement/i').locator('..').locator('input[type="checkbox"]')
    ).first();
  ...
  test('RGPD-CONSENT-05: Paramètres de consentement sont accessibles depuis Settings', async ({ page }) => {
    // Create and authenticate test user
    const { data: signUpData } = await createTestUser(testEmail, testPassword);
    userId = signUpData?.user?.id || null;
