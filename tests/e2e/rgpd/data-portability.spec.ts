...
test.describe('🔒 RGPD - Droit à la portabilité (Article 20)', () => {
  ...
  test('RGPD-PORT-03: Données sont compatibles avec import dans autres services', async ({ page }) => {
    // Create and authenticate test user
    const { data: signUpData } = await createTestUser(testEmail, testPassword);
    userId = signUpData?.user?.id || null;
  ...
  test('RGPD-PORT-04: Export inclut toutes les données nécessaires pour portabilité', async ({ page }) => {
    // Create and authenticate test user
    const { data: signUpData } = await createTestUser(testEmail, testPassword);
    userId = signUpData?.user?.id || null;
