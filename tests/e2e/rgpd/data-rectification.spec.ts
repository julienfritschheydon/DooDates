...
import { navigateToSettings, modifyUserProfile } from "../helpers/rgpd-helpers";
import {
  generateTestEmail,
  createTestUser,
  signInTestUser,
...
test.describe("🔒 RGPD - Droit de rectification (Article 16)", () => {
  ...
  test("RGPD-RECTIFY-04: Utilisateur peut corriger des erreurs dans ses données", async ({
    page,
  }) => {
    // Create and authenticate test user
    const { data: signUpData } = await createTestUser(testEmail, testPassword);
