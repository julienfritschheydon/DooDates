...
export async function robustNavigation(
  ...
  try {
    ...
    throw new Error(`Robust navigation failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function safeNavigationWithRetry(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(`⚠️ Tentative ${attempt} échouée:`, error instanceof Error ? error.message : String(error));
      
      if (attempt === maxRetries) {
        throw new Error(`Navigation failed after ${maxRetries} attempts: ${error instanceof Error ? error.message : String(error)}`);
      }
