...
test.describe("Rate Limiting RÉEL - HTTP 429", () => {
  test("should return HTTP 429 after exceeding test limits", async ({ request }) => {
    ...
    if (successCount === 0 && rateLimitCount === 0) {
      ...
      console.log(
        "   - Le rate limiting fonctionne probablement mais ne peut être testé sans auth valide",
      );
    ...
    if (successCount > 0) {
      ...
      console.log(
        `⚠️ Comportement inattendu: ${successCount} succès, ${rateLimitCount} rate limit`,
      );
