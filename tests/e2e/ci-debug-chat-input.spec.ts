...
test.describe('🔍 CI Debug - Chat Input Analysis', () => {
  ...
  test.beforeEach(async ({ page, browserName }) => {
    await setupTestEnvironment(page, browserName, {
      ...
      warmup: false,
      navigation: { path: '/DooDates/date-polls/workspace/date' },
      consoleGuard: {
        enabled: true,
        allowlist: [
  ...
  test('🔍 CI Debug - Complete page analysis for chat input @debug @ci-analysis', async ({ page, browserName }) => {
    ...
    for (let i = 0; i < Math.min(allTestIds.length, 10); i++) {
      ...
      log(`  🏷️ Element ${i}: data-testid="${testId}", tag=${tagName}, visible=${isVisible}, text="${textContent?.substring(0, 50)}..."`);
    }
    ...
    if (chatInputCount > 0) {
      ...
      for (let i = 0; i < Math.min(allInputs.length, 5); i++) {
        ...
        log(`  📝 Input ${i}: type="${inputType}", id="${inputId}", class="${inputClass}", placeholder="${inputPlaceholder}", visible=${isVisible}`);
      }
      ...
      for (let i = 0; i < Math.min(allTextareas.length, 5); i++) {
        ...
        log(`  📝 Textarea ${i}: id="${textareaId}", class="${textareaClass}", placeholder="${textareaPlaceholder}", visible=${isVisible}`);
      }
    }
