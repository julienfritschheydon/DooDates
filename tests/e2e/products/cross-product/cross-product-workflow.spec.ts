...
import {
  createPollInLocalStorage,
  createPollsAndVerifyInDashboard,
} from "../../helpers/poll-storage-helpers";
...
function calculateTotalPollsCreated(quota: {
  ...
  return (
    (quota.datePollsCreated || 0) +
    (quota.formPollsCreated || 0) +
    (quota.quizzCreated || 0) +
    (quota.availabilityPollsCreated || 0)
  );
}
...
test.describe("Cross-Product Workflow Tests", () => {
  ...
  async function waitForQuotaUpdate(
    ...
    throw new Error(
      `Timeout waiting for quota update. Expected: date=${expectedDatePolls}, form=${expectedFormPolls}`,
    );
  }
   ...
   */
  async function countPollsByType(
    page: any,
  ): Promise<{ date: number; form: number; availability: number }> {
  ...
  test("Créer Date Poll → Créer Form Poll → Vérifier quotas séparés", async ({
    page,
    browserName,
  }) => {
    ...
    console.log(
      `[TEST] Quota après Date Poll: date=${quota.datePollsCreated}, form=${quota.formPollsCreated}`,
    );
    ...
    console.log(
      `[TEST] Quota final: date=${quota.datePollsCreated}, form=${quota.formPollsCreated}, total=${totalPolls}`,
    );
  ...
  test("Utilisateur avec plusieurs produits → Vérifier isolation données", async ({
    page,
    browserName,
  }) => {
    const timeouts = getTimeouts(browserName);
  ...
  test("Modification service partagé → Vérifier impact tous produits", async ({
    page,
    browserName,
  }) => {
    const timeouts = getTimeouts(browserName);
    ...
    await waitForNetworkIdle(page, { browserName });
    await waitForReactStable(page, { browserName });
    await expect(page.getByText("Date Poll Shared Service Test")).toBeVisible({
    ...
    await waitForNetworkIdle(page, { browserName });
    await waitForReactStable(page, { browserName });
    await expect(page.getByText("Form Poll Shared Service Test")).toBeVisible({
      timeout: timeouts.element,
    });
    ...
    const updatedPolls = polls.filter(
      (p: any) => p.id === "test-date-poll-shared" || p.id === "test-form-poll-shared",
    );
    ...
    await waitForNetworkIdle(page, { browserName });
    await waitForReactStable(page, { browserName });
    await expect(page.getByText("Date Poll Shared Service Test (Modifié)")).toBeVisible({
      timeout: timeouts.element,
    });
    ...
    await waitForNetworkIdle(page, { browserName });
    await waitForReactStable(page, { browserName });
    await expect(page.getByText("Form Poll Shared Service Test (Modifié)")).toBeVisible({
      timeout: timeouts.element,
    });
  });
  ...
  test("Changement quota → Vérifier application correcte", async ({ page, browserName }) => {
    ...
    console.log(
      `[TEST] Quota final après changement: date=${quota.datePollsCreated}, form=${quota.formPollsCreated}, total=${totalPolls2}`,
    );
  });
});
