# ❓ FAQ - Questions Fréquentes

Réponses aux questions les plus courantes sur DooDates.

---

## 📋 Catégories

1. [Général](#général)
2. [Compte et Connexion](#compte-et-connexion)
3. [Création de Sondages](#création-de-sondages)
4. [Vote et Réponses](#vote-et-réponses)
5. [Analytics IA](#analytics-ia)
6. [Résultats et Export](#résultats-et-export)
7. [Quotas et Limites](#quotas-et-limites)
8. [Sécurité et Confidentialité](#sécurité-et-confidentialité)
9. [Tarification](#tarification)
10. [Support Technique](#support-technique)

---

## 🌐 Général

### Qu'est-ce que DooDates ?

DooDates est une plateforme de création de sondages et formulaires propulsée par l'IA. Créez des sondages en quelques secondes simplement en parlant naturellement à notre assistant.

---

### DooDates est-il gratuit ?

**Oui !** DooDates propose une version gratuite complète avec sondages illimités.

Pour plus de fonctionnalités avancées (conversations IA illimitées, intégrations, white-label, etc.), consultez notre **[page Tarifs](/pricing)** pour découvrir nos offres Pro et Premium.

---

### Quelle est la différence avec Doodle, SurveyMonkey ou Google Forms ?

| Feature | DooDates | Concurrents |
|---------|----------|-------------|
| **IA conversationnelle** | ✅ Créer en parlant naturellement | ❌ Formulaires classiques |
| **Analytics IA** | ✅ Insights automatiques | ⚠️ Basique ou payant |
| **Export gratuit** | ✅ 4 formats illimités | ⚠️ Limité ou payant |
| **Réponses illimitées** | ✅ Toujours | ⚠️ Limité (25-100) |
| **Interface moderne** | ✅ Mobile-first 2025 | ⚠️ Datée (2010s) |
| **Simulation réponses** | ✅ IA génère des tests | ❌ Non disponible |

---

### Sur quels appareils DooDates fonctionne-t-il ?

DooDates fonctionne sur :
- 💻 **Ordinateurs** : Windows, Mac, Linux (tous navigateurs modernes)
- 📱 **Smartphones** : iOS, Android (Safari, Chrome, Firefox)
- 🖥️ **Tablettes** : iPad, Android

**Interface responsive** adaptée automatiquement à votre écran.

---

## 👤 Compte et Connexion

### Dois-je créer un compte ?

**Non, c'est optionnel !**

**Mode invité :**
- ✅ Créer des sondages immédiatement
- ⚠️ Données stockées localement (appareil uniquement)
- ⚠️ 5 conversations IA maximum

**Avec compte (recommandé) :**
- ✅ Synchronisation multi-appareils
- ✅ 50 conversations IA/mois
- ✅ Historique complet
- ✅ Sauvegarde cloud

---

### Comment créer un compte ?

1. Cliquez sur **"Se connecter"** en haut à droite
2. Choisissez **"Continuer avec Google"**
3. Autorisez DooDates à accéder à votre profil Google
4. Vous êtes connecté !

**Pas besoin de mot de passe** (authentification via Google).

---

### Puis-je utiliser DooDates sans compte Google ?

Actuellement, l'authentification se fait uniquement via Google. 

**Alternatives en développement :**
- Email/mot de passe classique (Q1 2026)
- Microsoft/LinkedIn (Q2 2026)
- Apple Sign-In (Q2 2026)

En attendant, utilisez le **mode invité** sans compte.

---

### Que se passe-t-il si je supprime mon compte ?

**Suppression de compte :**
1. Vos sondages sont **supprimés définitivement** après 30 jours
2. Les liens de partage deviennent **inactifs**
3. Les réponses déjà collectées sont **effacées**
4. Vos conversations IA sont **supprimées**

**⚠️ Action irréversible** après 30 jours (délai de rétractation).

**Alternative :** Archivez vos sondages au lieu de supprimer votre compte.

---

## 📊 Création de Sondages

### Combien de sondages puis-je créer ?

**Illimité !** Quel que soit votre plan :
- 🆓 **Gratuit** : Sondages illimités
- 💼 **Pro** : Sondages illimités
- 🚀 **Premium** : Sondages illimités

**Seule limite :** Conversations IA (5, 50, ou illimité selon plan).

---

### Combien de questions puis-je poser dans un formulaire ?

**Limite technique :** 100 questions par formulaire

**Recommandation :**
- ✅ **Court** (3-7 questions) : Taux de complétion ~85%
- ⚠️ **Moyen** (8-15 questions) : Taux ~65%
- ❌ **Long** (15+) : Taux ~40%

**Astuce :** Utilisez la **logique conditionnelle** pour réduire le nombre de questions affichées par répondant.

---

### Puis-je modifier un sondage après l'avoir publié ?

**Oui, mais avec précautions :**

**Sans votes/réponses :**
- ✅ Modification totale autorisée

**Avec < 5 réponses :**
- ✅ Modification possible
- ⚠️ Avertissement affiché

**Avec 5-20 réponses :**
- ⚠️ Ajout de questions OK
- ⚠️ Modification du texte OK
- ❌ Suppression déconseillée (données perdues)

**Avec 20+ réponses :**
- ✅ Ajout de nouvelles questions
- ❌ Suppression bloquée (intégrité des données)

**Recommandation :** Testez avec la **Simulation de réponses** avant de publier !

---

### Comment créer un sondage anonyme ?

**Pour un sondage de dates :**
```
1. Créez votre sondage
2. Paramètres → "Votes anonymes" → Activé
3. Les noms des votants ne s'afficheront pas
```

**Pour un formulaire :**
```
1. Ne demandez pas d'informations personnelles (nom, email)
2. Paramètres → "Collecte anonyme" → Activé
3. Pas de tracking IP
```

---

### Puis-je dupliquer un sondage existant ?

**Oui !**

```
1. Dashboard → Trouvez votre sondage
2. Menu ⋮ → "Dupliquer"
3. Un nouveau sondage identique est créé
4. Modifiez-le à votre guise
```

**Cas d'usage :**
- Enquête mensuelle récurrente
- A/B testing (2 versions d'un questionnaire)
- Template réutilisable

---

## 🗳️ Vote et Réponses

### Puis-je voter plusieurs fois ?

**Cela dépend des paramètres du créateur :**

**Mode par défaut :** 1 vote par appareil (cookie)
- ❌ Impossible de revoter depuis le même navigateur
- ✅ Possible depuis un autre appareil/navigateur

**Mode "Compte requis" :** 1 vote par utilisateur
- ❌ Impossible de revoter même avec un autre appareil

**Mode "Votes illimités" :** Oui
- ✅ Vote possible plusieurs fois (rare, cas spécifiques)

---

### Puis-je modifier mon vote après l'avoir soumis ?

**Sondages de dates :**
- ✅ **Oui**, retournez sur le lien → Modifiez vos disponibilités

**Formulaires :**
- ❌ **Non** par défaut (soumission unique)
- ✅ Possible si le créateur a activé "Modification autorisée"

---

### Dois-je donner mon email pour voter ?

**Non, sauf si :**
- Le créateur a rendu un champ "Email" obligatoire
- Vous souhaitez recevoir une copie de vos réponses

**Par défaut :** Vote 100% anonyme sans email.

---

### Pourquoi ne vois-je pas certaines questions ?

**Logique conditionnelle activée !**

Certaines questions s'affichent uniquement selon vos réponses précédentes.

**Exemple :**
```
Q1 : Êtes-vous satisfait ? → Non

Q2 (apparaît) : Que pouvons-nous améliorer ?
Q3 (masquée) : Nous recommanderiez-vous ?
```

C'est normal et permet des formulaires plus courts et pertinents.

---

## 🤖 Analytics IA

### Combien de conversations IA puis-je utiliser ?

| Plan | Quota |
|------|-------|
| **🆓 Invité** | 5 conversations (à vie) |
| **👤 Gratuit** | 50/mois (reset le 1er) |
| **💼 Pro** | Illimité |
| **🚀 Premium** | Illimité |

**1 conversation = 1 interaction IA** (insight, quick query, question libre)

---

### Qu'est-ce qui consomme une conversation IA ?

**Actions consommant 1 crédit :**
- 🔍 Génération d'insights automatiques (3 crédits à l'ouverture)
- ❓ Clic sur une Quick Query (1 crédit)
- 💬 Question libre posée (1 crédit)

**Actions gratuites (0 crédit) :**
- Consultation des graphiques standards
- Export des résultats
- Partage des liens

---

### Les insights IA sont-ils fiables ?

**Fiabilité selon le nombre de réponses :**

| Réponses | Fiabilité | Marge d'erreur |
|----------|-----------|----------------|
| < 10 | ⚠️ Faible | ±30% |
| 10-30 | ⚫ Moyenne | ±15% |
| 30-100 | ✅ Bonne | ±10% |
| 100+ | ✅✅ Excellente | ±5% |

**Conseil :** Attendez au moins 30 réponses avant de vous fier aux insights.

---

### L'IA a-t-elle accès à tous mes sondages ?

**Non !** L'IA analyse uniquement :
- ✅ Le sondage que vous consultez
- ✅ Ses réponses et métadonnées
- ❌ PAS vos autres sondages
- ❌ PAS vos données personnelles

**Isolation complète** entre chaque sondage.

---

## 📤 Résultats et Export

### Qui peut voir les résultats ?

**3 niveaux de visibilité** (configurables par le créateur) :

1. **🔒 Créateur uniquement**
   - Vous seul pouvez voir les résultats

2. **👥 Participants (après vote)**
   - Toute personne ayant voté peut voir les résultats

3. **🌍 Public**
   - Tout le monde avec le lien peut voir

**Par défaut :** Créateur uniquement.

---

### Puis-je exporter les résultats ?

**Oui !** 4 formats disponibles gratuitement :

| Format | Usage |
|--------|-------|
| **CSV** | Excel, analyse de données |
| **PDF** | Rapport imprimable |
| **JSON** | Intégration technique |
| **Markdown** | Documentation |

**Pas de limite** : Export illimité même en version gratuite.

---

### Les résultats sont-ils en temps réel ?

**Oui !** Les résultats se mettent à jour automatiquement :
- ✅ Nouveaux votes affichés instantanément
- ✅ Graphiques actualisés en direct
- ✅ Pas besoin de rafraîchir la page

**Technologie :** WebSockets pour la synchronisation temps réel.

---

## 📏 Quotas et Limites

### Combien de réponses puis-je recevoir ?

**Illimité !** Quel que soit votre plan :
- 🆓 **Gratuit** : Réponses illimitées
- 💼 **Pro** : Réponses illimitées
- 🚀 **Premium** : Réponses illimitées

**Pas de plafond** : Recevez 10, 100, 1000, ou 100 000 réponses sans surcoût.

---

### Y a-t-il une limite de temps pour collecter des réponses ?

**Non par défaut**, sauf si vous configurez une deadline :

```
Paramètres → Deadline de vote → Activée
→ Choisissez date et heure de clôture automatique
```

**Sinon :** Sondage actif indéfiniment jusqu'à clôture manuelle.

---

### Combien de temps mes sondages sont-ils conservés ?

**Durée de conservation :**

| Type | Durée |
|------|-------|
| **Sondages actifs** | Indéfinie |
| **Sondages clôturés** | 2 ans d'inactivité |
| **Sondages archivés** | 5 ans |
| **Compte supprimé** | 30 jours (puis suppression définitive) |

**Conseil :** Exportez vos données importantes régulièrement.

---

## 🔒 Sécurité et Confidentialité

### Mes données sont-elles sécurisées ?

**Oui, DooDates utilise :**
- 🔒 **Chiffrement HTTPS** (TLS 1.3) pour toutes les connexions
- 🔐 **Chiffrement au repos** (AES-256) pour les données stockées
- 🛡️ **Infrastructure sécurisée** (Vercel + Supabase)
- ✅ **Conformité RGPD** complète
- ❌ **Pas de vente de données** à des tiers

**Certifications :** ISO 27001 (en cours), SOC 2 Type II (planifié)

---

### DooDates respecte-t-il le RGPD ?

**Oui à 100% !**

**Droits garantis :**
- ✅ **Droit d'accès** : Export complet de vos données
- ✅ **Droit de rectification** : Modification de vos infos
- ✅ **Droit à l'effacement** : Suppression de compte
- ✅ **Droit à la portabilité** : Export JSON/CSV
- ✅ **Droit d'opposition** : Opt-out analytics/marketing

**Contact DPO :** dpo@doodates.com

---

### L'IA utilise-t-elle mes données pour s'entraîner ?

**Non !** Vos données :
- ✅ Sont analysées en temps réel pour VOS insights uniquement
- ❌ Ne sont PAS stockées après l'analyse
- ❌ Ne servent PAS à entraîner les modèles IA
- ❌ Ne sont PAS partagées avec OpenAI/Google

**Garantie contractuelle** dans nos CGU.

---

### Puis-je anonymiser les réponses a posteriori ?

**Oui partiellement :**

```
Résultats → Paramètres → "Anonymiser les noms"
→ Les noms sont masqués dans l'affichage (mais pas supprimés)
```

**Anonymisation complète :**
```
Exporter en CSV → Supprimer colonne "Nom" dans Excel
```

**⚠️ Attention :** L'anonymisation est irréversible si vous supprimez les données sources.

---

## 💰 Tarification

### Quelles sont les différences entre les plans ?

| Feature | Gratuit | Pro (10€) | Premium (100€) |
|---------|---------|-----------|----------------|
| **Sondages** | ✅ Illimité | ✅ Illimité | ✅ Illimité |
| **Réponses** | ✅ Illimité | ✅ Illimité | ✅ Illimité |
| **Export** | ✅ 4 formats | ✅ 4 formats | ✅ Tous formats |
| **Conversations IA** | 50/mois | ✅ Illimité | ✅ Illimité |
| **Analytics IA avancés** | ❌ | ✅ | ✅ |
| **Simulation réponses** | ❌ | ✅ | ✅ |
| **Intégrations** | ❌ | ⚠️ Basiques | ✅ Complètes |
| **White-label** | ❌ | ❌ | ✅ |
| **Support prioritaire** | Email (48h) | Email (8h) | Chat (2h) |

---

### Puis-je passer de Gratuit à Pro en cours de mois ?

**Oui !** Le changement est **immédiat** :
```

---

### Y a-t-il des frais cachés ?

**Aucun !** Tarification 100% transparente :
- ❌ Pas de frais de setupS
- ❌ Pas de frais d'export
- ❌ Pas de frais de résiliation

**Ce que vous payez :** Uniquement l'abonnement mensuel (10€ ou 100€).

---

### Puis-je annuler mon abonnement ?

**Oui, à tout moment !**

```
Compte → Abonnement → "Annuler"
→ Effet immédiat, pas de préavis requis
```

**Après annulation :**
- ✅ Accès jusqu'à la fin du mois payé
- ✅ Vos sondages restent accessibles
- ⚠️ Retour au plan Gratuit (quota IA réduit)

**Pas de frais** d'annulation.

---

## 🛠️ Support Technique

### Comment contacter le support ?

**3 canaux disponibles :**

1. **💬 Chat en ligne** (coin bas droit)
   - Réponse : < 5 minutes (heures ouvrables)
   - Disponible : Lun-Ven 9h-18h CET

2. **📧 Email** : support@doodates.com
   - Gratuit : < 48h
   - Pro : < 8h
   - Premium : < 2h

3. **📖 Documentation** : docs.doodates.com
   - 17 guides complets
   - FAQ exhaustive
   - Tutoriels vidéo

---

### J'ai trouvé un bug, que faire ?

**Signaler un bug :**

```
1. Cliquez sur l'icône 🐛 en bas de page
2. Décrivez le problème
3. (Optionnel) Ajoutez une capture d'écran
4. Envoyez !
```

**Informations utiles :**
- Navigateur et version
- URL de la page concernée
- Étapes pour reproduire

**Récompense :** Bugs critiques récompensés (crédit compte Pro).

---

### Où puis-je suggérer une fonctionnalité ?

**3 méthodes :**

1. **Widget Feedback** (icône 💡)
2. **Roadmap publique** : roadmap.doodates.com
   - Votez sur les features proposées
   - Proposez les vôtres
3. **Email** : feedback@doodates.com

**Fonctionnalités les plus votées** sont priorisées en développement.

---

### DooDates est-il disponible dans ma langue ?

**Langues actuellement disponibles :**
- 🇫🇷 Français (complet)
- 🇬🇧 Anglais (complet)

**Langues en développement :**
- 🇪🇸 Espagnol (Q1 2026)
- 🇩🇪 Allemand (Q2 2026)
- 🇮🇹 Italien (Q2 2026)

**Contribution :** Intéressé pour traduire ? → translate@doodates.com

---

## 🔗 Liens Utiles

- 📖 **Documentation complète** : [Retour à l'index](./README.md)
- 🚀 **Guide de démarrage** : [Premiers pas](./01-Guide-Demarrage-Rapide.md)
- 💡 **Cas d'usage** : [Exemples concrets](./11-Cas-Usage.md)
- 📧 **Contact support** : support@doodates.com

---

**Cette FAQ n'a pas répondu à votre question ?**  
Contactez-nous : support@doodates.com

---

**[← Personnalisation](./13-Personnalisation.md) | [Accueil](./README.md) | [Glossaire →](./15-Glossaire.md)**

---

**© DooDates - Documentation officielle**

