...
function testCircularDependency() {
  ...
  log(
    `  Test 1.2: Boucle Q1→Q2→Q1 → ${result2 ? "✅ PASS" : "❌ FAIL"}`,
    result2 ? "green" : "red",
  );
  ...
  log(
    `  Test 1.3: Boucle Q1→Q2→Q3→Q1 → ${result3 ? "✅ PASS" : "❌ FAIL"}`,
    result3 ? "green" : "red",
  );
...
function testDependsOnPrevious() {
  ...
  log(
    `  Test 2.1: Q2 dépend Q1 (avant) → ${result1 ? "✅ PASS" : "❌ FAIL"}`,
    result1 ? "green" : "red",
  );

  // Test 2.2: Q1 dépend de Q2 (invalide)
  const rules2 = [
    { questionId: "q1", dependsOn: "q2", showIf: { operator: "equals", value: "Oui" } },
  ];
  const result2 = dependsOnPreviousQuestion(rules2, questions, "q1");
  log(
    `  Test 2.2: Q1 dépend Q2 (après) → ${!result2 ? "✅ PASS" : "❌ FAIL"}`,
    !result2 ? "green" : "red",
  );

  // Test 2.3: Q3 dépend de Q1 (valide)
  const rules3 = [
    { questionId: "q3", dependsOn: "q1", showIf: { operator: "equals", value: "Oui" } },
  ];
  const result3 = dependsOnPreviousQuestion(rules3, questions, "q3");
  log(
    `  Test 2.3: Q3 dépend Q1 (avant) → ${result3 ? "✅ PASS" : "❌ FAIL"}`,
    result3 ? "green" : "red",
  );
...
function testEvaluateRule() {
  ...
  const evaluateRule = (rule, answers) => {
    ...
    switch (rule.showIf.operator) {
      ...
      case "contains": {
        if (!rule.showIf.value) return false;
        const expectedValues = Array.isArray(rule.showIf.value)
          ? rule.showIf.value
          : [rule.showIf.value];
  ...
  log(
    `  Test 3.1a: equals "Oui" avec réponse "Oui" → ${result1a ? "✅ PASS" : "❌ FAIL"}`,
    result1a ? "green" : "red",
  );
  log(
    `  Test 3.1b: equals "Oui" avec réponse "Non" → ${!result1b ? "✅ PASS" : "❌ FAIL"}`,
    !result1b ? "green" : "red",
  );
  ...
  log(
    `  Test 3.2a: contains "Option A" avec ["A","B"] → ${result2a ? "✅ PASS" : "❌ FAIL"}`,
    result2a ? "green" : "red",
  );
  log(
    `  Test 3.2b: contains "Option A" avec ["B","C"] → ${!result2b ? "✅ PASS" : "❌ FAIL"}`,
    !result2b ? "green" : "red",
  );
  ...
  log(
    `  Test 3.3a: notEquals "Non" avec "Oui" → ${result3a ? "✅ PASS" : "❌ FAIL"}`,
    result3a ? "green" : "red",
  );
  log(
    `  Test 3.3b: notEquals "Non" avec "Non" → ${!result3b ? "✅ PASS" : "❌ FAIL"}`,
    !result3b ? "green" : "red",
  );
  ...
  log(
    `  Test 3.4a: isEmpty avec "" → ${result4a ? "✅ PASS" : "❌ FAIL"}`,
    result4a ? "green" : "red",
  );
  log(
    `  Test 3.4b: isEmpty avec "text" → ${!result4b ? "✅ PASS" : "❌ FAIL"}`,
    !result4b ? "green" : "red",
  );
  log(
    `  Test 3.4c: isEmpty sans réponse → ${result4c ? "✅ PASS" : "❌ FAIL"}`,
    result4c ? "green" : "red",
  );
  ...
  log(
    `  Test 3.5a: isNotEmpty avec "text" → ${result5a ? "✅ PASS" : "❌ FAIL"}`,
    result5a ? "green" : "red",
  );
  log(
    `  Test 3.5b: isNotEmpty avec "" → ${!result5b ? "✅ PASS" : "❌ FAIL"}`,
    !result5b ? "green" : "red",
  );
...
function testShouldShowQuestion() {
  ...
  log(
    `  Test 4.1: Question sans règle → ${result1 ? "✅ PASS" : "❌ FAIL"}`,
    result1 ? "green" : "red",
  );
  ...
  log(
    `  Test 4.2a: Q3bis avec Q3="Non" → ${result2a ? "✅ PASS" : "❌ FAIL"}`,
    result2a ? "green" : "red",
  );
  log(
    `  Test 4.2b: Q3bis avec Q3="Oui" → ${!result2b ? "✅ PASS" : "❌ FAIL"}`,
    !result2b ? "green" : "red",
  );
  ...
  log(
    `  Test 4.3: Q3bis sans réponse Q3 → ${!result3 ? "✅ PASS" : "❌ FAIL"}`,
    !result3 ? "green" : "red",
  );
...
function testCaseCrews() {
  ...
  log(
    `  Test 5.1: Q3="Non, pas vraiment" → Q3bis visible → ${result1 ? "✅ PASS" : "❌ FAIL"}`,
    result1 ? "green" : "red",
  );
  ...
  log(
    `  Test 5.2: Q3="Oui, très bien..." → Q3bis masqué → ${!result2 ? "✅ PASS" : "❌ FAIL"}`,
    !result2 ? "green" : "red",
  );
  ...
  log(
    `  Test 5.3: Q3="Moyennement" → Q3bis masqué → ${!result3 ? "✅ PASS" : "❌ FAIL"}`,
    !result3 ? "green" : "red",
  );
