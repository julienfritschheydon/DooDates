...
const optionsRate = result.questionsWithOptions > 0 
  ? Math.round((result.questionsWithOptions / (result.totalQuestions - result.textQuestions)) * 100)
  : 0;

log(`\n📈 Taux de détection questions: ${detectionRate}%`, detectionRate >= 90 ? 'green' : detectionRate >= 70 ? 'yellow' : 'red');
log(`📋 Taux d'extraction options: ${optionsRate}%`, optionsRate >= 90 ? 'green' : optionsRate >= 70 ? 'yellow' : 'red');
