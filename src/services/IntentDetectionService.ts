...
const PATTERNS = {
  ...
  ADD_DATE: /ajout(?:e|er)\s+(le\s+)?(\d{4}-\d{2}-\d{2}|\d{1,2}(?:\/\d{1,2}(?:\/\d{4})?|\s+(?:janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre)\s+\d{4})?)/i,
  
  // "retire/supprime/enlève [jour de la semaine]"
  REMOVE_DAY: /(?:retire|supprime|enl[èe]ve)\s+(le\s+)?(lundi|mardi|mercredi|jeudi|vendredi|samedi|dimanche)/i,
  
  // "retire/supprime/enlève [date]" - Formats multiples
  REMOVE_DATE: /(?:retire|supprime|enl[èe]ve)\s+(le\s+)?(\d{4}-\d{2}-\d{2}|\d{1,2}(?:\/\d{1,2}(?:\/\d{4})?|\s+(?:janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre)\s+\d{4})?)/i,
  ...
  TIMESLOT_1: /ajout(?:e|er)\s+(\d{1,2})h?(\d{2})?\s*-\s*(\d{1,2})h?(\d{2})?\s+le\s+(\d{1,2}(?:\/\d{1,2}(?:\/\d{4})?)?)/i,
  
  // "ajoute de [heure] à [heure] le [date]"
  TIMESLOT_2: /ajout(?:e|er)\s+de\s+(\d{1,2})h?(\d{2})?\s+[àa]\s+(\d{1,2})h?(\d{2})?\s+le\s+(\d{1,2}(?:\/\d{1,2}(?:\/\d{4})?)?)/i,
  
  // "ajoute le [date] de [heure] à [heure]"
  TIMESLOT_3: /ajout(?:e|er)\s+le\s+(\d{1,2}(?:\/\d{1,2}(?:\/\d{4})?)?)\s+de\s+(\d{1,2})h?(\d{2})?\s+[àa]\s+(\d{1,2})h?(\d{2})?/i,
...
export class IntentDetectionService {
  ...
  private static buildTimeslotIntent(
    ...
    if (dateStr.includes('/')) {
      ...
      const month = parts[1] ? parts[1].padStart(2, '0') : String(new Date().getMonth() + 1).padStart(2, '0');
  ...
  static detectSimpleIntent(
    ...
    if (match) {
      ...
      const monthTextMatch = dateStr.match(/(\d{1,2})\s+(janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre)\s+(\d{4})/i);
      if (monthTextMatch) {
