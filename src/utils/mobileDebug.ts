...
class MobileDebugger {
  ...
  public init(options: { logToConsole?: boolean } = {}): void {
    ...
    const eventTypes: (keyof WindowEventMap)[] = ['touchstart', 'touchmove', 'touchend', 'touchcancel'];
  ...
  private detectConflicts(): void {
    ...
    for (let i = 0; i < recentEvents.length - 1; i++) {
      ...
      const relatedEvents = recentEvents.slice(i + 1).filter(
        e => e.target === startEvent.target
      );
      ...
      for (const event of relatedEvents) {
        ...
        if (event.type === 'touchmove' && duration > this.LONG_PRESS_DURATION) {
          console.warn(`[MobileDebug] Conflit détecté: long-press (${duration}ms) suivi d'un mouvement`, {
            startEvent,
            moveEvent: event,
            duration
          });
