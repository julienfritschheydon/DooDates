...
const formPollPrompts: FormPollPromptSpec[] = [
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    category: "simples",
    input:
      "Crée un questionnaire de satisfaction client avec au moins 3 questions : une question à choix unique, une à choix multiples, et une question de réponse libre",
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    category: "nps",
    input:
      "Crée un questionnaire avec une question de type NPS (Net Promoter Score) demandant la probabilité de recommandation de notre service sur une échelle de 0 à 10",
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    category: "event",
    input:
      "Crée un questionnaire pour recueillir les préférences des participants : type de nourriture préféré, horaire préféré, et allergies alimentaires",
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    category: "complex",
    input:
      "Crée un questionnaire d'enquête client avec : une matrice d'évaluation pour noter plusieurs aspects, des questions de choix multiples avec maximum 3 réponses, et une question email avec validation",
...
function scoreFormPollTest(
  ...
  maxScore: number;
  violations: string[];
  breakdown: { type: number; questionCount: number; questionTypes: number; validations: number };
} {
  const maxScore = 4.0;
  let score = 0;
  ...
  if (prompt.expectedQuestionTypes && prompt.expectedQuestionTypes.length > 0) {
    ...
    if (ratio === 1.0) {
      ...
      const missing = prompt.expectedQuestionTypes.filter(
        (expectedType) => !questionTypes.includes(expectedType),
      );
  ...
  if (prompt.expectedValidationTypes && prompt.expectedValidationTypes.length > 0) {
    ...
    if (ratio === 1.0) {
      ...
      const missing = prompt.expectedValidationTypes.filter(
        (expectedValidation) => !validationTypes.includes(expectedValidation),
      );
...
describe("Gemini Form Polls Tests", () => {
  ...
  filteredPrompts.forEach((prompt) => {
    it(`[${prompt.priority || "MOYENNE"}] ${prompt.description}`, async () => {
      ...
      console.log(
        `   📊 Breakdown: Type=${scoring.breakdown.type.toFixed(1)}, Questions=${scoring.breakdown.questionCount.toFixed(1)}, Types=${scoring.breakdown.questionTypes.toFixed(1)}, Validations=${scoring.breakdown.validations.toFixed(1)}`,
      );
  ...
  afterAll(() => {
    ...
    console.log(
      `   📈 Score: ${totalScore.toFixed(2)}/${maxTotalScore.toFixed(0)} (${percentage.toFixed(1)}%)`,
    );
    console.log("=".repeat(70));
  });
});
