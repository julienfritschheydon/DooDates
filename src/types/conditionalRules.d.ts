...
export interface ConditionalValidation {
     ...
     */
    dependsOnPreviousQuestion: (rules: ConditionalRule[], questions: Array<{
        id: string;
    }>, questionId: string) => boolean;
    /**
     * Vérifie qu'une valeur existe dans les options de la question
     */
    valueExistsInOptions: (questionId: string, value: string, questions: Array<{
        id: string;
        options?: Array<{
            label: string;
        }>;
...
export interface ConditionalEvaluator {
     ...
     */
    shouldShowQuestion: (questionId: string, rules: ConditionalRule[], answers: Record<string, string | string[]>) => boolean;
    /**
