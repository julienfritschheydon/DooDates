...
export function useQuota(config: UseQuotaConfig = {}): UseQuotaReturn {
  ...
  const [authModalTrigger, setAuthModalTrigger] = useState<AuthIncentive['type']>('conversation_limit');
  ...
  const autoDeletion = useMemo((): AutoDeletionInfo => {
    ...
    const nextCleanup = lastCleanup ? new Date(lastCleanup.getTime() + (24 * 60 * 60 * 1000)) : new Date();
    const daysUntilNextCleanup = Math.max(0, Math.ceil((nextCleanup.getTime() - Date.now()) / (24 * 60 * 60 * 1000)));
  ...
  }, [enableAutoDeletion, isAuthenticated, storage.conversations.data, limits.retentionDays, lastCleanup]);
  ...
  const authIncentive = useMemo((): AuthIncentive => {
    ...
    const shouldShow = !hasReachedMaxViews && !isDismissed && 
      (status.conversations.isAtLimit || status.conversations.isNearLimit || 
       status.polls.isAtLimit || status.storage.isAtLimit || quotaInfo.used >= 3);
  ...
  const executeAutoDeletion = useCallback(async (): Promise<number> => {
    ...
    for (const conversation of autoDeletion.candidates) {
      try {
        ...
        console.warn(`Failed to auto-delete conversation ${conversation.id}:`, error);
      }
    }
  ...
  // Auto-cleanup effect
  useEffect(() => {
    if (!autoDeletion.enabled || autoDeletion.candidateCount === 0 || autoDeletion.daysUntilNextCleanup > 0) {
      return;
    }
  ...
  }, [autoDeletion.enabled, autoDeletion.candidateCount, autoDeletion.daysUntilNextCleanup, executeAutoDeletion]);
