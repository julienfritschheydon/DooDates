    ...
    async (firstMessage: AutoSaveMessage): Promise<Conversation> => {
      ...
      try {
        ...
        if (!DISABLE_SUPABASE_CONVERSATIONS && effectiveUserId) {
          ...
          // Utiliser un titre temporaire qui sera automatiquement régénéré par triggerTitleGeneration
    ...
    (conversationId: string, messages: ConversationMessage[]) => {
      ...
      titleGenerationTimeoutRef.current = setTimeout(async () => {
        try {
          ...
          if (!conversation) {
            console.log('🔍 [TitleGen] Conversation non trouvée:', conversationId);
            return;
          }
          ...
          if (shouldRegenerateTitle(conversation.title, hasCustomTitle, messages.length)) {
            ...
            console.log('🔍 [TitleGen] Résultat génération:', titleResult);
            if (titleResult.success) {
              try {
                // Mettre à jour le titre de la conversation via le service
                await TitleService.updateEntityTitle('conversation', conversationId, titleResult.title);
                
                // Si la conversation a un poll associé, mettre à jour son titre aussi
                if (conversation.pollId) {
                  try {
                    await TitleService.updateEntityTitle('poll', conversation.pollId, titleResult.title);
                    console.log('✅ [TitleGen] Titre du poll mis à jour via TitleService:', {
                      pollId: conversation.pollId,
                      newTitle: titleResult.title
                    });
                  } catch (pollError) {
                    logger.error('Erreur lors de la mise à jour du titre du poll', 'general', { pollId: conversation.pollId, error: pollError });
                  }
                }
                ...
                console.log('✅ [TitleGen] Titre mis à jour via TitleService:', titleResult.title);
              ...
              console.log('❌ [TitleGen] Échec génération titre:', titleResult.failureReason);
            }
          } else {
            console.log('⏭️ [TitleGen] Génération non nécessaire');
          }
        } catch (error) {
          console.log('❌ [TitleGen] Erreur:', error);
