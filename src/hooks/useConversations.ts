...
export function useConversations(config: UseConversationsConfig = {}) {
  ...
  const queryKeys = {
    ...
    messages: (conversationId: string) => ['messages', user?.id || 'guest', conversationId] as const,
    ...
    queryFn: async ({ pageParam = 0 }) => {
      try {
        ...
        let filteredConversations = conversations.filter(conv => {
          ...
          if (filters.isFavorite !== undefined && conv.isFavorite !== filters.isFavorite) return false;
    ...
    onMutate: async (conversationId) => {
      ...
      queryClient.setQueryData(queryKeys.infinite, (old: any) => {
        ...
        return {
          ...
          pages: old.pages.map((page: any) => ({
            ...page,
            conversations: page.conversations.filter((conv: Conversation) => conv.id !== conversationId),
            totalCount: page.totalCount - 1
          }))
        };
      });
  ...
  const addMessageMutation = useMutation({
    ...
    onMutate: async ({ conversationId, message }) => {
      ...
      // Update messages
      queryClient.setQueryData(queryKeys.messages(conversationId), (old: ConversationMessage[] | undefined) => {
        return old ? [...old, optimisticMessage] : [optimisticMessage];
      });

      // Update conversation message count and updatedAt
      queryClient.setQueryData(queryKeys.conversation(conversationId), (old: Conversation | undefined) => {
        if (!old) return old;
        return {
          ...old,
    ...
    onSuccess: (newMessage) => {
      queryClient.invalidateQueries({ queryKey: queryKeys.messages(newMessage.conversationId) });
      queryClient.invalidateQueries({ queryKey: queryKeys.conversation(newMessage.conversationId) });
      queryClient.invalidateQueries({ queryKey: queryKeys.infinite });
    },
  });
  ...
  useEffect(() => {
    ...
    const handleRealtimeUpdate = (payload: any) => {
      ...
      switch (eventType) {
        ...
        case 'UPDATE':
          ...
          break;
        case 'DELETE':
          queryClient.removeQueries({ queryKey: queryKeys.conversation(oldRecord.id) });
          queryClient.invalidateQueries({ queryKey: queryKeys.infinite });
          break;
      }
    };
    ...
    console.log('Real-time sync would be enabled here for user:', user.id);

    return () => {
      // Cleanup real-time subscriptions
      console.log('Real-time sync cleanup');
    };
