...
describe('useConversationSearch Logic', () => {
  ...
  const mockConversations: Conversation[] = [
    createMockConversation('1', 'Meeting with team', 'Discuss project roadmap', 'active', ['work', 'meeting'], false, new Date('2024-01-01')),
    createMockConversation('2', 'Personal notes', 'Remember to buy groceries', 'completed', ['personal'], true, new Date('2024-01-02')),
    createMockConversation('3', 'Bug report analysis', 'Critical bug in authentication system', 'active', ['bug', 'urgent'], false, new Date('2024-01-03')),
    createMockConversation('4', 'Weekly planning', 'Plan tasks for next week', 'archived', ['planning'], false, new Date('2024-01-04'), 'poll-123'),
    createMockConversation('5', 'Code review notes', 'Review pull request #456', 'active', ['code', 'review'], true, new Date('2024-01-05'))
  ];
  ...
  describe('Filtering functionality', () => {
    it('should filter by conversation status', () => {
      ...
      const completedConversations = mockConversations.filter(conv => conv.status === 'completed');
    ...
    it('should filter by favorite status', () => {
      ...
      const nonFavoriteConversations = mockConversations.filter(conv => conv.isFavorite === false);
  ...
  describe('Edge cases', () => {
    it('should handle conversations with empty fields', () => {
      ...
      const messageMatch = emptyConversation.firstMessage.toLowerCase().includes(query.toLowerCase());
  ...
  describe('Filter combinations', () => {
    it('should handle all filters set to restrictive values', () => {
      ...
      const results = mockConversations.filter(conv => {
        if (restrictiveFilters.status !== 'all' && conv.status !== restrictiveFilters.status) return false;
        if (restrictiveFilters.isFavorite !== undefined && conv.isFavorite !== restrictiveFilters.isFavorite) return false;
