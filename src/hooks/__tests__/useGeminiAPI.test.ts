...
describe('useGeminiAPI', () => {
  ...
  describe('generatePoll', () => {
    ...
    it('should trim message before processing', async () => {
      ...
      expect(mockGeminiService.generatePollFromText).toHaveBeenCalledWith('Organise une réunion demain');
      expect(response?.success).toBe(true);
    });

    it('should set loading state during request', async () => {
      let resolvePromise: (value: { success: boolean; data?: any; message: string; error?: string }) => void;
      const promise = new Promise<{ success: boolean; data?: any; message: string; error?: string }>((resolve) => {
    ...
    it('should handle quota exceeded error', async () => {
      ...
      expect(response).toEqual({
        success: false,
        error: 'Limite de quota atteinte. Veuillez réessayer plus tard ou vous connecter pour plus de requêtes.',
        errorType: 'quota',
      });
      expect(result.current.error).toBe('Limite de quota atteinte. Veuillez réessayer plus tard ou vous connecter pour plus de requêtes.');
    ...
    it('should handle network error', async () => {
      ...
      expect(result.current.error).toBe('Problème de connexion réseau. Vérifiez votre connexion internet.');
    ...
    it('should handle parsing error', async () => {
      ...
      expect(result.current.error).toBe('Erreur lors de l\'analyse de la réponse. Veuillez reformuler votre demande.');
    });
    ...
    it('should handle unknown error', async () => {
      ...
      expect(response).toEqual({
        success: false,
        error: 'Désolé, je n\'ai pas pu traiter votre demande. Pouvez-vous reformuler ou réessayer ?',
        errorType: 'unknown',
      });
      expect(result.current.error).toBe('Désolé, je n\'ai pas pu traiter votre demande. Pouvez-vous reformuler ou réessayer ?');
    });
