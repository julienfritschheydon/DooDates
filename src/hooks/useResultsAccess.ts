 ...
 */
export function useResultsAccess(poll: Poll | null, hasVoted: boolean = false): ResultsAccessStatus {
  const { user } = useAuth();
  ...
  const visibility = (poll.settings as any)?.resultsVisibility || 'public';
  ...
  if (visibility === 'creator-only') {
    if (!user) {
      return {
        allowed: false,
        reason: 'restricted',
        message: 'Seul le créateur peut voir les résultats. Connectez-vous si vous êtes le créateur.'
      };
    }
    if (!isCreator) {
      return {
