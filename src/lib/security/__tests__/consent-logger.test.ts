...
describe('ConsentLoggerService', () => {
  ...
  describe('logCookieConsent', () => {
    it('should log cookie consent for all types', () => {
      ...
      expect(cookieLogs.some(log => log.consentType === 'essential' && log.consentValue === true)).toBe(true);
      expect(cookieLogs.some(log => log.consentType === 'analytics' && log.consentValue === false)).toBe(true);
    });
  });
  ...
  describe('exportUserData', () => {
    it('should export user data with summary', () => {
      consentLogger.logConsent({ action: 'consent_given', userId: 'user1', consentType: 'analytics', consentValue: true });
      consentLogger.logConsent({ action: 'consent_withdrawn', userId: 'user1', consentType: 'marketing', consentValue: false });
  ...
  describe('getConsentStats', () => {
    it('should return comprehensive stats', () => {
      consentLogger.logConsent({ action: 'consent_given', userId: 'user1', consentType: 'analytics' });
      consentLogger.logConsent({ action: 'consent_given', userId: 'user2', consentType: 'essential' });
      consentLogger.logConsent({ action: 'consent_withdrawn', userId: 'user1', consentType: 'marketing' });
...
describe('Consent Logger Integration', () => {
  it('should handle GDPR workflow', () => {
    ...
    expect(exportData.consentLogs).toHaveLength(6); // 4 cookies + 1 access + 1 deletion
    expect(exportData.summary.consentGiven).toBe(4);
    expect(exportData.summary.consentWithdrawn).toBe(1);
    expect(exportData.summary.dataAccessRequests).toBe(1);
    ...
    expect(deleteResult.deletedCount).toBe(6);
