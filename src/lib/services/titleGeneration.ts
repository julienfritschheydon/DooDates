...
function extractKeyPhrases(text: string, language: 'fr' | 'en'): string[] {
  ...
  const schedulingPatterns = language === 'fr' ? [
  ...
  for (const word of words) {
    ...
    if (/^[A-ZÀ-ÿ][a-zà-ÿ''-]{1,}$/.test(word)) {
      ...
      const commonWords = ['Bonjour', 'Hello', 'Hi', 'Parfait', 'Perfect', 'Très', 'Very', 'Nous', 'We', 'Je', 'Il', 'Elle', 'Vous', 'The', 'This', 'That'];
  ...
  const timePattern = /\b(?:\d{1,2}[h:]\d{0,2}|\d{1,2}\s*(?:h|heures?|pm|am)|\d{1,2}pm|\d{1,2}am)\b/gi;
...
function generateTitleFromPhrases(
  ...
  const namesPhrases = phrases.filter(p => 
    /^[A-ZÀ-ÿ][a-zà-ÿ''-]+$/.test(p)
  );

  // Prioritize time expressions (improved pattern)
  const timePhrases = phrases.filter(p => 
    /\b(?:\d{1,2}[h:]\d{0,2}|\d{1,2}\s*(?:h|heures?|pm|am)|\w+day|\w+di|matin|après-midi|soir|midi|morning|afternoon|evening)\b/i.test(p)
  );
