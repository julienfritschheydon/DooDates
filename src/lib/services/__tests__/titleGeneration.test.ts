...
describe('titleGeneration', () => {
  describe('generateConversationTitle', () => {
    it('should generate title from scheduling conversation in French', () => {
      const messages = [
        createMessage('user', 'Bonjour, je voudrais organiser une réunion avec Paul et Marie pour mardi prochain à 14h'),
        createMessage('assistant', 'Parfait ! Je vais vous aider à organiser cette réunion avec Paul et Marie pour mardi à 14h. Avez-vous une préférence pour la durée ?'),
        createMessage('user', 'Une heure devrait suffire, c\'est pour discuter du projet DooDates'),
        createMessage('assistant', 'Excellente idée ! Une réunion d\'une heure sur le projet DooDates.')
      ];
    ...
    it('should generate title from scheduling conversation in English', () => {
      const messages = [
        createMessage('user', 'Hi, I need to schedule a meeting with John and Sarah for next Wednesday at 3pm'),
        createMessage('assistant', 'I\'ll help you schedule that meeting with John and Sarah for Wednesday at 3pm. What\'s the meeting about?'),
    ...
    it('should respect custom length constraints', () => {
      const messages = [
        createMessage('user', 'Réunion avec Paul et Marie mardi 14h pour projet DooDates très important'),
        createMessage('assistant', 'Parfait pour cette réunion importante avec Paul et Marie mardi à 14h concernant le projet DooDates')
      ];
    ...
    it('should prioritize scheduling phrases over generic content', () => {
      const messages = [
        createMessage('user', 'Bonjour comment allez-vous ? Je voudrais organiser une réunion avec Paul mardi'),
        createMessage('assistant', 'Bonjour ! Je vais bien merci. Parfait pour organiser cette réunion avec Paul mardi')
      ];
  ...
  describe('edge cases and robustness', () => {
    ...
    it('should limit number of names to avoid overly long titles', () => {
      const messages = [
        createMessage('user', 'Réunion avec Paul, Marie, Jean, Pierre, Sophie, Claire, Michel et Anne'),
      ...
      const nameCount = (result.title.match(/Paul|Marie|Jean|Pierre|Sophie|Claire|Michel|Anne/gi) || []).length;
