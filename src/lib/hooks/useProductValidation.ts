...
export const useProductValidation = <T extends Record<string, unknown>>(
  ...
  const validateField = useCallback(
    (field: keyof T, value: unknown): string | null => {
      // ...
      nonEmptyArray: z.array(z.unknown()).min(1, "Au moins un élément est requis"),
          ...
          try {
            ...
            for (const rule of rules) {
              ...
              try {
                ...
                if (!result.success) {
                  errors[fieldName] =
                    rule.message || result.error.issues[0]?.message || "Champ invalide";
                }
              } catch (err) {
  ...
  export const commonValidationSchemas = {
    ...
    url: z.string().url("L'URL n'est pas valide").optional(),

    date: z.string().datetime("La date n'est pas valide"),

    positiveNumber: z.number().positive("Le nombre doit être positif"),

    nonEmptyArray: z.array(z.any()).min(1, "Au moins un élément est requis"),
  };
