...
import { generateTimeSlots, toggleTimeSlotForDate, formatSelectedDateHeader } from '../timeSlotFunctions';
...
describe('Calendar Integration Tests', () => {
  ...
  describe('Time Slot Management', () => {
    it('should toggle time slots correctly', () => {
      ...
      expect(timeSlotsByDate['2025-07-01'][0]).toEqual({ hour: 9, minute: 0, enabled: true });
      
      // Toggle it off
      timeSlotsByDate = toggleTimeSlotForDate('2025-07-01', 9, 0, timeSlotsByDate);
      expect(timeSlotsByDate['2025-07-01'][0]).toEqual({ hour: 9, minute: 0, enabled: false });
    ...
    it('should handle multiple time slots for same date', () => {
      ...
      expect(timeSlotsByDate['2025-07-01']).toContainEqual({ hour: 9, minute: 0, enabled: true });
      expect(timeSlotsByDate['2025-07-01']).toContainEqual({ hour: 14, minute: 30, enabled: true });
    });
    ...
    it('should handle multiple dates', () => {
      ...
      expect(timeSlotsByDate['2025-07-01']).toContainEqual({ hour: 9, minute: 0, enabled: true });
      expect(timeSlotsByDate['2025-07-02']).toContainEqual({ hour: 14, minute: 0, enabled: true });
    });
  });
  ...
  describe('Complete Workflow', () => {
    it('should support a complete poll creation workflow', () => {
      ...
      pollConfig.dates.forEach(date => {
        ...
        expect(timeSlotsByDate[date]).toContainEqual({ hour: 9, minute: 0, enabled: true });
        expect(timeSlotsByDate[date]).toContainEqual({ hour: 14, minute: 0, enabled: true });
      });
