...
export class EnhancedGeminiService {
  ...
  private analyzeTemporalInput(userInput: string): SimpleTemporalAnalysis {
    ...
    if (text.includes('semaine prochaine')) {
      const today = new Date();
      for (let i = 1; i <= 5; i++) { // Lundi à vendredi semaine suivante
        const date = new Date(today);
  ...
  private buildCounterfactualPrompt(userInput: string, analysis: SimpleTemporalAnalysis): string {
    ...
    if (userInput.includes('lundi') || userInput.includes('mardi')) {
      counterfactualQuestions.push('Si on changeait le jour demandé, est-ce que le contexte resterait cohérent ?');
    }
    
    if (userInput.includes('matin') || userInput.includes('soir')) {
      counterfactualQuestions.push('Si on inversait matin/soir, les horaires seraient-ils logiques ?');
    }
...
Réponds SEULEMENT avec le JSON validé.`;
   ...
   */
  private parseAndValidateResponse(text: string, analysis: SimpleTemporalAnalysis): EnhancedPollSuggestion | null {
    try {
      ...
      if (jsonMatch) {
        ...
        if (parsed.title && parsed.dates && Array.isArray(parsed.dates)) {
          ...
          if (validationErrors.length > 0) {
            console.warn('❌ Échec validation counterfactual:', validationErrors);
