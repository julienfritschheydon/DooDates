...
export const ConversationSchema = z.object({
  ...
  title: z
    ...
    .max(
      CONVERSATION_LIMITS.MAX_TITLE_LENGTH,
      `Le titre ne peut pas dépasser ${CONVERSATION_LIMITS.MAX_TITLE_LENGTH} caractères`,
    ),
...
export const ConversationSearchFiltersSchema = z
  .object({
...
export const ConversationQuotaSchema = z
  .object({
  ...
  .refine((data) => data.currentCount <= data.maxConversations, {
...
export function validateAddMessageRequest(
  data: unknown,
...
export function validateConversationQuota(
  data: unknown,
...
export function sanitizeMessageContent(content: string): string {
  return content.trim().slice(0, CONVERSATION_LIMITS.MAX_CONVERSATION_SIZE);
}
