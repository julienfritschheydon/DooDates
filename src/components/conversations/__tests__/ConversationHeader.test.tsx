...
describe('ConversationHeader', () => {
  ...
  describe('Rendu de base', () => {
    ...
    it('should not show resume button for archived conversations', () => {
      const archivedConversation = createMockConversation({ status: 'archived' });
      render(<ConversationHeader {...defaultProps} conversation={archivedConversation} />);
      
      expect(screen.queryByRole('button', { name: /Reprendre la conversation/ })).not.toBeInTheDocument();
    });
  });
  ...
  describe('Badge bidirectionnel sondage', () => {
    it('should show poll link badge when relatedPollId exists', () => {
      const conversationWithPoll = createMockConversation({ relatedPollId: 'poll-123' });
    ...
    it('should call onViewPoll when poll badge is clicked', async () => {
      ...
      render(<ConversationHeader {...defaultProps} conversation={conversationWithPoll} onViewPoll={onViewPoll} />);
    ...
    it('should call onViewPoll when poll action button is clicked', async () => {
      ...
      const pollButton = screen.getByRole('button', { name: /Voir le sondage lié/ });
  ...
  describe('Navigation bidirectionnelle', () => {
    it('should call onResume when resume button is clicked', async () => {
      ...
      const resumeButton = screen.getByRole('button', { name: /Reprendre la conversation/ });
  ...
  describe('Mode compact', () => {
    ...
    it('should show emojis in compact mode for actions', () => {
      const conversationWithPoll = createMockConversation({ relatedPollId: 'poll-123' });
      render(<ConversationHeader {...defaultProps} conversation={conversationWithPoll} compact={true} />);
  ...
  describe('Traductions i18n', () => {
    ...
    it('should show English poll badge when language=en', () => {
      const conversationWithPoll = createMockConversation({ relatedPollId: 'poll-123' });
      render(<ConversationHeader {...defaultProps} conversation={conversationWithPoll} language="en" />);
    ...
    it('should show English favorite badge when language=en', () => {
      const favoriteConversation = createMockConversation({ isFavorite: true });
      render(<ConversationHeader {...defaultProps} conversation={favoriteConversation} language="en" />);
  ...
  describe('États de statut', () => {
    it('should show correct styling for completed status', () => {
      const completedConversation = createMockConversation({ status: 'completed' });
