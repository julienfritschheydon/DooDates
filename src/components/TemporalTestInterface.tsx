...
const TemporalTestInterface: React.FC = () => {
  ...
  const validateResult = (input: string, analysis: any, geminiResult: any): { score: number; passed: boolean } => {
    let score = 100;
    const text = input.toLowerCase();
        ...
        <CardContent className="space-y-4">
          ...
          {results.length > 0 && (
            <Alert>
              ...
              <AlertDescription>
                Score moyen: <span className={getScoreColor(averageScore)}>{averageScore.toFixed(1)}%</span>
                {' • '}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div>Confiance: <Badge variant="outline">{(result.analysis.confidence * 100).toFixed(1)}%</Badge></div>
                    <div>Type: <Badge>{result.analysis.temporalType}</Badge></div>
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div>Titre: <span className="font-mono text-xs bg-gray-100 p-1 rounded">{result.geminiResult?.title}</span></div>
                    <div>Dates: {result.geminiResult?.dates.length} générées</div>
                    <div>Créneaux: {result.geminiResult?.timeSlots.length}</div>
                    <div>Type: <Badge>{result.geminiResult?.type}</Badge></div>
                  </div>
                </div>
              </div>
              ...
              {(result.analysis.conflicts.length > 0 || result.analysis.suggestions.length > 0) && (
                <div className="space-y-2">
                  {result.analysis.conflicts.length > 0 && (
                    <div>
                      <h5 className="text-sm font-semibold text-red-600 mb-1">Conflits détectés:</h5>
                      <ul className="text-xs space-y-1">
                        {result.analysis.conflicts.map((conflict, i) => (
                          <li key={i} className="text-red-600">• {conflict}</li>
                        ))}
                      </ul>
                    </div>
                  )}
