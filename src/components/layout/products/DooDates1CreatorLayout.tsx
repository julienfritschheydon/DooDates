...
export function DooDates1CreatorLayout({ children }: DooDates1CreatorLayoutProps) {
    ...
    const loadData = useCallback(() => {
        try {
            ...
            const datePolls = polls.filter(p => p.type === 'date');
    ...
    return (
        <div className="flex h-screen bg-[#1e1e1e] overflow-hidden">
            ...
            {/* DooDates1 Sidebar */}
            <div
                className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#1a1a1a] transform transition-transform duration-300 flex flex-col border-r border-blue-900/30 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"
                    }`}
            >
                {/* Header */}
                <div className="flex-shrink-0 flex items-center justify-between p-4 border-b border-blue-900/30 bg-gradient-to-r from-blue-600/10 to-blue-500/10">
                    <h2 className="text-lg font-semibold text-blue-400">DooDates1</h2>
                    <button
                                    ...
                                    {conversations.slice(0, 10).map((conv) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className="w-full flex items-start gap-3 p-3 hover:bg-blue-900/10 rounded-lg transition-colors text-left mb-1"
                                            >
                                                <div className="p-2 bg-blue-900/20 rounded-lg flex-shrink-0">
                                                    <LazyIconWrapper Icon={MessageSquare} className="w-4 h-4 text-blue-400" />
                                                </div>
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-[#0a0a0a]">
                <div className="h-14 flex-shrink-0 bg-[#0a0a0a] flex items-center px-4 border-b border-blue-900/20">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-xl font-medium text-blue-400">DooDates1</h1>
                    </div>
                </div>
