...
export function DooDates3CreatorLayout({ children }: DooDates3CreatorLayoutProps) {
    ...
    const loadData = useCallback(() => {
        try {
            ...
            const availabilityPolls = polls.filter(p => p.type === 'availability');
    ...
    return (
        <div className="flex h-screen bg-[#1e1e1e] overflow-hidden">
            ...
            <div
                className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#1a1a1a] transform transition-transform duration-300 flex flex-col border-r border-emerald-900/30 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"
                    }`}
            >
                <div className="flex-shrink-0 flex items-center justify-between p-4 border-b border-emerald-900/30 bg-gradient-to-r from-emerald-600/10 to-emerald-500/10">
                    <h2 className="text-lg font-semibold text-emerald-400">DooDates3</h2>
                    <button
                    ...
                    <>
                        <div className="px-4 pb-4 space-y-2">
                            <button
                                ...
                                className="w-full flex items-center gap-3 px-4 py-3 text-white bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 rounded-lg transition-colors font-medium"
                            >
                                <LazyIconWrapper Icon={Clock} className="w-5 h-5" />
                                <span>Nouvelle Dispo</span>
                            </button>
                            ...
                            <button
                                onClick={() => {
                                    navigate("/availability-polls/pricing");
                                    if (isMobile) setIsSidebarOpen(false);
                                }}
                                className="w-full flex items-center gap-3 px-4 py-3 text-gray-300 bg-[#2a2a2a] hover:bg-[#3a3a3a] rounded-lg transition-colors font-medium"
                            >
                                    ...
                                    {conversations.slice(0, 10).map((conv) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className="w-full flex items-start gap-3 p-3 hover:bg-emerald-900/10 rounded-lg transition-colors text-left mb-1"
                                            >
                                                <div className="p-2 bg-emerald-900/20 rounded-lg flex-shrink-0">
                                                    <LazyIconWrapper Icon={MessageSquare} className="w-4 h-4 text-emerald-400" />
                                                </div>
                                                    ...
                                                    {relatedPoll && (
                                                        <div className="flex items-center gap-1.5 mt-1">
                                                            ...
                                                            <p className="text-xs text-gray-500 truncate">
                                                                Disponibilité : {relatedPoll.title}
                                                            </p>
                                                        </div>
                                                    )}
                ...
                <div className="p-3 border-t border-gray-800/50">
                    {user ? (
                        <div className="px-3 py-3 bg-gradient-to-br from-emerald-900/30 to-emerald-800/30 rounded-xl border border-emerald-700/30">
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className="flex-1 px-2 py-2 text-xs font-medium text-gray-200 hover:text-white hover:bg-emerald-700/50 rounded-lg transition-all flex items-center justify-center gap-1.5 border border-emerald-700/30"
                                >
                        ...
                        <div className="px-3 py-3 bg-gradient-to-br from-gray-800/60 to-gray-900/60 rounded-xl border border-gray-700/50">
                            ...
                            <button
                                ...
                                className="w-full px-3 py-2.5 text-xs font-semibold text-white bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg hover:shadow-emerald-500/20"
                            >
                                Se connecter
                            </button>
                        </div>
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-[#0a0a0a]">
                <div className="h-14 flex-shrink-0 bg-[#0a0a0a] flex items-center px-4 border-b border-emerald-900/20">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-xl font-medium text-emerald-400">DooDates3</h1>
                    </div>
                </div>
