...
const Dashboard: React.FC = () => {
  ...
  const filteredPolls = polls.filter((poll) => {
    // Normaliser le statut: si undefined ou null, considérer comme "draft"
    const pollStatus = poll.status || "draft";
    const matchesFilter = filter === "all" || pollStatus === filter;
    const matchesSearch = poll.title.toLowerCase().includes(searchQuery.toLowerCase());

    // Debug logs pour comprendre le filtrage
    if (filter !== "all") {
      console.log(
        `🔍 Filter: ${filter}, Poll: ${poll.title}, Status: ${pollStatus} (original: ${poll.status}), Matches: ${matchesFilter}`,
      );
    }
