...
export default function ConditionalRuleEditor({
  ...
  const getRuleGlobalIndex = (localIndex: number) => {
    return existingRules.findIndex(
      (r, idx) => r.questionId === questionId && existingRules.slice(0, idx + 1).filter((r2) => r2.questionId === questionId).length === localIndex + 1
    );
  };
  ...
  return (
    <div className="space-y-2 p-3 bg-blue-50 border border-blue-200 rounded-md">
      ...
      {currentRules.map((rule, localIdx) => {
        ...
        if (!isEditing) {
          return (
            <div key={localIdx} className="text-xs text-gray-700 bg-white p-2 rounded border border-gray-200">
              <span className="text-blue-700 font-medium">"{dependsOnQuestion?.title || "Question"}"</span>{" "}
              <span>
