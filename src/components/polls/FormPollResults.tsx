...
import { getPollBySlugOrId, Poll, getFormResults, getFormResponses, getRespondentId } from "@/lib/pollStorage";
...
export default function FormPollResults({ idOrSlug }: Props) {
  ...
  const computed = useMemo(() => {
    const map: Record<string, { kind: string; counts?: Record<string, number>; samples?: string[]; count?: number }> = {};
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <ResultsLayout
        ...
        subtitle={
          <>
            {totalRespondents} participant{totalRespondents > 1 ? "s" : ""} • {questions.length} question{questions.length > 1 ? "s" : ""}
          </>
        }
        ...
        kpis={[
          ...
          { label: "Questions à choix", value: Object.keys(countsByQuestion).length },
        ]}
      >
                    ...
                    {(q.options || []).map((opt: any) => {
                      ...
                      return (
                        <div key={opt.id} className="flex items-center gap-3">
                          <div className="w-48 text-sm text-gray-700">{opt.label || "Option"}</div>
                          ...
                          <div className="w-24 text-right text-sm text-gray-700">{count} ({pct}%)</div>
                        </div>
                      );
                    })}
                  </div>
                      ...
                      return list.length ? (
                        <>
                          {list.map((txt: string, i: number) => (
                            <div key={i} className="text-sm text-gray-800 border rounded px-3 py-2 bg-gray-50">
                              {txt}
                            </div>
                          ))}
                          {all.length > (stats?.samples?.length || 0) && (
                            <div>
                              <button
                                ...
                                onClick={() =>
                                  setExpandedText((prev) => ({ ...prev, [qid]: !isExpanded }))
                                }
                              >
                ...
                <div key={getRespondentId(r)} className="py-3">
                  <div className="flex items-center justify-between">
                    <div className="font-medium">
                      {r.respondentName && r.respondentName.trim() ? r.respondentName.trim() : "Anonyme"}
                    </div>
                    <div className="text-xs text-gray-500">{new Date(r.created_at).toLocaleString()}</div>
                  </div>
                  <div className="mt-2 grid gap-2 sm:grid-cols-2">
                    {r.items.map((it: any) => {
                      const q = questions.find((qq: any) => qq.id === it.questionId) as any;
                      if (!q) return null;
