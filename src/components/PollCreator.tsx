...
import { ConflictList } from "./availability/ConflictList";
...
const PollCreator: React.FC<PollCreatorProps> = ({
  ...
  // État pour la détection de conflits (Nouveau Hook)
  const { conflictingSlots, checkConflicts, isChecking: isAnalyzingCalendar } = useCalendarConflicts();

  // Effet pour déclencher la vérification des conflits
  useEffect(() => {
    logger.info("🔄 PollCreator: Checking conflict requirements", "calendar", {
      hasDates: state.selectedDates.length > 0,
      hasCalendarService: !!googleCalendarRef.current,
      datesCount: state.selectedDates.length
    });

    if (state.selectedDates.length > 0 && googleCalendarRef.current) {
      const token = localStorage.getItem("provider_token");
      logger.info("🔑 PollCreator: Checking token", "calendar", { hasToken: !!token });

      if (token) {
        googleCalendarRef.current.setAccessToken(token);

        // Construire la liste des créneaux à vérifier
        const datesToCheck: { date: string; start?: string; end?: string }[] = [];

        state.selectedDates.forEach(date => {
          const slots = timeSlotsByDate[date]?.filter(s => s.enabled);

          if (slots && slots.length > 0) {
            // Si des créneaux horaires sont définis, on vérifie chaque créneau
            slots.forEach(slot => {
              datesToCheck.push({
                date,
                start: `${String(slot.hour).padStart(2, '0')}:${String(slot.minute).padStart(2, '0')}:00`,
                end: `${String(slot.hour).padStart(2, '0')}:${String(slot.minute + (state.timeGranularity || 60)).padStart(2, '0')}:00` // Approximation de fin
                // TODO: Mieux calculer la fin avec la granularité
              });
            });
          } else {
            // Sinon on vérifie la journée entière
            datesToCheck.push({ date });
          }
        });

        logger.info("🚀 PollCreator: Triggering checkConflicts", "calendar", { datesToCheck });

        // Debounce léger pour éviter trop d'appels
        const timer = setTimeout(() => {
          checkConflicts(datesToCheck, googleCalendarRef.current!);
        }, 500);

        return () => clearTimeout(timer);
      } else {
        logger.warn("⚠️ PollCreator: No provider token found", "calendar");
      }
    }
  }, [state.selectedDates, timeSlotsByDate, state.timeGranularity, checkConflicts]);

  // Ancienne logique handleAnalyzeCalendar supprimée (remplacée par useEffect)

  // Détection automatique avec debounce
  // Ancien useEffect de debounce supprimé (intégré au nouveau)

  // Anciens handlers de conflits supprimés
