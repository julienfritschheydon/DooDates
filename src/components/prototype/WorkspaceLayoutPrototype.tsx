import { X, Menu, Plus, Calendar, ClipboardList, FileText, MessageSquare, Sparkles } from "lucide-react";
  ...
  useEffect(() => {
    try {
      ...
      const uniquePolls = Array.from(
        polls.reduce((map, poll) => {
          const existing = map.get(poll.id);
          if (!existing || new Date(poll.updated_at || poll.created_at) > new Date(existing.updated_at || existing.created_at)) {
            map.set(poll.id, poll);
          }
          return map;
        }, new Map<string, Poll>()).values()
      );
  ...
  return (
    <>
      <div className="flex h-screen overflow-hidden bg-[#1e1e1e]">
        ...
        <div className={`${sidebarCollapsed ? 'w-16' : 'w-64'} bg-[#1e1e1e] flex-shrink-0 flex flex-col border-r border-gray-700 transition-all duration-300`}>
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium text-white truncate flex items-center gap-1">
                                ...
                                {hasPoll && (
                                  <span className="text-blue-400" title="Sondage créé">💬</span>
                                )}
                              </p>
                              <p className="text-xs text-gray-400">
                                {conv.createdAt && formatDistanceToNow(new Date(conv.createdAt), { 
                                  addSuffix: true, 
                                  locale: fr 
                                })}
                              </p>
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-white truncate flex items-center gap-1">
                              ...
                              {findRelatedConversation(poll) && (
                                <span className="text-blue-400" title="Créé par IA">💬</span>
                              )}
                            </p>
                            <p className="text-xs text-gray-400">
                              {poll.type === "form" ? "Formulaire" : "Sondage dates"} • {poll.created_at && formatDistanceToNow(new Date(poll.created_at), { 
                                addSuffix: true, 
                                locale: fr 
                              })}
                            </p>
        ...
        <div className={`flex flex-col bg-[#0a0a0a] transition-all duration-300 ${
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <svg className="w-5 h-5 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
                  ...
                  <path strokeLinecap="round" strokeLinejoin="round" d="15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
              </button>
              <button 
                className="p-2 hover:bg-gray-800 rounded-lg transition-colors"
