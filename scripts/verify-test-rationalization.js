...
const oldFileRemoved = !fs.existsSync(path.join(process.cwd(), 'tests/integration/real-supabase-simplified.test.ts'));
console.log(`${oldFileRemoved ? '✅' : '❌'} Ancien fichier supprimé (real-supabase-simplified.test.ts)`);
...
try {
  ...
  console.log(`📏 Lignes de code : ~${lines} (vs 650 auparavant = -${Math.round((650-lines)/650*100)}%)`);
  console.log(`🧪 Nombre de tests : ${testCount} (vs 26 auparavant = -${Math.round((26-testCount)/26*100)}%)`);
} catch (error) {
...
try {
  execSync('npx tsc --noEmit tests/integration/api-security-performance.spec.ts tests/integration/shared/test-helpers.ts', {
    stdio: 'pipe'
  });
...
console.log('npx playwright test tests/integration/api-security-performance.spec.ts --project=chromium');
