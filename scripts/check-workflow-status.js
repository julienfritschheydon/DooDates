...
function checkStatus() {
  ...
  if (!fs.existsSync(STATUS_FILE)) {
    console.log('⚠️  Fichier de statut non trouvé. Le monitoring n\'a peut-être pas encore été exécuté.');
    console.log('💡 Exécutez le workflow "8️⃣ Workflow Monitoring & Health Report" pour générer le rapport.\n');
    return;
  }
  ...
  console.log(`📅 Dernière mise à jour: ${lastUpdate.toLocaleString('fr-FR', { timeZone: 'Europe/Paris' })}`);
  ...
  if (hoursSinceUpdate > 2) {
    console.log('⚠️  Le rapport est ancien (>2h). Le monitoring devrait s\'exécuter toutes les heures.');
