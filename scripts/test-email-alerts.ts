...
async function testCalculSuppressions(): Promise<any[]> {
  ...
  try {
    const warnings = await retentionService.calculateUpcomingDeletions(
      "test-user-123",
      testSettings,
    );
    ...
    warnings.forEach((warning) => {
      console.log(
        `   - ${warning.type}: ${warning.itemCount} éléments dans ${warning.daysUntilDeletion} jours`,
      );
    });
...
async function testGenerationEmail(): Promise<void> {
  ...
  try {
    ...
    const emailContent = await (
      retentionService as unknown as {
        generateEmailContent?: (
          warning: typeof testWarning,
        ) => Promise<{ subject: string; html: string } | null>;
      }
    ).generateEmailContent?.(testWarning);
    ...
    const mockUsers = [
      {
        ...
        chat_retention: "30-days",
        poll_retention: "12-months",
      ...
      },
      {
        ...
        chat_retention: "indefinite",
        poll_retention: "6-years",
        auto_delete_enabled: true,
        email_notifications: true,
      },
      {
      ...
      if (user.auto_delete_enabled && user.email_notifications) {
        ...
        if (imminentWarnings.length > 0) {
          ...
          imminentWarnings.forEach((w) => {
            console.log(
              `   - ${w.type}: ${w.itemCount} éléments dans ${w.daysUntilDeletion} jours`,
            );
          });
  ...
  try {
    ...
    if (typeof window !== "undefined" && window.localStorage) {
      ...
      localStorage.setItem(
        "doodates_email_notifications",
        testSettings.emailNotifications.toString(),
      );
      localStorage.setItem(
        "doodates_allow_data_improvement",
        testSettings.allowDataForImprovement.toString(),
      );
