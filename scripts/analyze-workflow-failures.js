        ...
        for (let j = i + 1; j < Math.min(i + 50, lines.length); j++) {
          ...
          if (/AssertionError|Expected.*but got|Expected.*received/i.test(nextLine)) {
            ...
            for (let k = j + 1; k < Math.min(j + 10, lines.length); k++) {
              ...
              if (contextLine.trim().startsWith('Expected:') ||
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (testMatch1) {
      ...
      for (let j = i + 1; j < Math.min(i + 30, lines.length); j++) {
        ...
        if (/Error:|Timeout|element\(s\) not found|expect\(.*\)\.(toContainText|toBeVisible)/i.test(nextLine)) {
          errorContext.push(nextLine);
          
          // Collecter le contexte (Locator, Expected, etc.)
          for (let k = j + 1; k < Math.min(j + 15, lines.length); k++) {
            const contextLine = lines[k];
            if (contextLine.includes('Locator:') ||
...
async function analyzeLastFailure() {
  ...
  const workflowsResponse = await fetch(
    `${GITHUB_API_BASE}/repos/${REPO}/actions/workflows`,
    {
      headers: {
        'Authorization': `Bearer ${GITHUB_TOKEN}`,
        'Accept': 'application/vnd.github.v3+json',
  ...
  const workflow = workflowsData.workflows?.find(w => w.name === '2️⃣ Develop → Main (Auto-merge)');
  ...
  for (const job of failedJobs) {
    ...
    if (job.name.includes('unit') || job.name.includes('test')) {
      ...
      const summaryMatch = logs.match(/Test Files\s+(\d+)\s+failed[^\n]*\n[^\n]*Tests\s+(\d+)\s+failed/i);
      if (summaryMatch) {
        console.log(`   📊 Résumé: ${summaryMatch[1]} fichier(s) de test échoué(s), ${summaryMatch[2]} test(s) échoué(s)\n`);
      }
      
      // Chercher les patterns FAIL
      const failMatches = [...logs.matchAll(/FAIL\s+(.+\.test\.(?:ts|tsx|js|jsx))\s+\((\d+)\s+test.*?\)/gi)];
      if (failMatches.length > 0) {
        ...
        for (const match of failMatches) {
          ...
          const fileSection = nextFileStart > 0 
          ...
          if (testFailures.length > 0) {
            for (const testMatch of testFailures) {
              ...
              const assertionMatch = errorSection.match(/AssertionError[^\n]*\n([^\n]*Expected[^\n]*)\n([^\n]*Received[^\n]*)/);
              if (assertionMatch) {
      ...
      if (testFailures.length > 0) {
        ...
        const errorMatches = [...logs.matchAll(/Error:\s+.*expect\(.*\)\.(toContainText|toBeVisible)[^\n]*/gi)];
        if (errorMatches.length > 0) {
          ...
          const errorLines = logs.split('\n').filter(l => /Error|Timeout|expect|toContainText/i.test(l)).slice(0, 20);
