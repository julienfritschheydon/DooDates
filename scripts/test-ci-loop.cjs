 ...
 * Teste NODE_ENV=development avec les mêmes paramètres que CI
 */
...
const testCommand = `npx playwright test tests/e2e/ci-debug-chat-input.spec.ts --project=chromium --reporter=list`;
...
function runTest() {
  ...
  const child = spawn('npx', ['playwright', 'test', 'tests/e2e/ci-debug-chat-input.spec.ts', '--project=chromium', '--reporter=list'], {
    env: envVars,
    stdio: 'inherit',
    shell: true,
    cwd: process.cwd()
  ...
  child.on('close', (code) => {
    ...
    if (code === 0) {
      ...
      if (attempt < maxAttempts) {
        attempt++;
        console.log(`⏳ Attente 3s avant la tentative ${attempt}...`);
