...
files.forEach((filePath) => {
  ...
  let fixed = content
    ...
    .replace(
      /page\.goto\('form-polls\/\$\{([^}]+)\}\/results', \{ waitUntil: 'domcontentloaded' \}\);/g,
      "page.goto(`form-polls/${$1}/results`, { waitUntil: 'domcontentloaded' });",
    )
    .replace(
      /page\.goto\("\/quizz\/\$\{MOCK_QUIZZ\.slug\}\/vote", \{ waitUntil: "domcontentloaded" \}\);/g,
      'page.goto(`/quizz/${MOCK_QUIZZ.slug}/vote`, { waitUntil: "domcontentloaded" });',
    )
