...
function runTests() {
  ...
  try {
    ...
    if (result.includes('production-smoke') &&
  ...
  try {
    ...
    if (result.includes('runtime') &&
        result.includes('Ajouter vérification null/undefined')) {
      console.log('✅ PASS');
      passed++;
    } else {
      console.log('❌ FAIL - Analyse incorrecte:', result.substring(0, 200));
  ...
  try {
    ...
    if (result.includes('Critiques : 1') &&
        result.includes('Résoudre immédiatement')) {
      console.log('✅ PASS');
      passed++;
    } else {
  ...
  try {
    ...
    if (result.includes('Erreur non cataloguée') &&
