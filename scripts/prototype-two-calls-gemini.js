...
async function compareApproaches() {
  ...
  for (const testCase of difficultCases) {
    ...
    console.log(`\n   📊 Comparaison:`);
    console.log(
      `      Un appel: ${comparison.singleCall.datesCount} dates, ${comparison.singleCall.timeSlotsCount} créneaux (${time1}ms)`,
    );
    console.log(
      `      Deux appels: ${comparison.twoCalls.datesCount} dates, ${comparison.twoCalls.timeSlotsCount} créneaux (${time2}ms)`,
    );
    ...
    if (result2?.understanding) {
      ...
      console.log(
        `         - Jours: ${result2.understanding.temporalInfo?.daysOfWeek?.join(", ") || "aucun"}`,
      );
      console.log(
        `         - Mois: ${result2.understanding.temporalInfo?.months?.join(", ") || "aucun"}`,
      );
      console.log(
        `         - Contrainte "ou": ${result2.understanding.temporalInfo?.constraints?.isOrConstraint || false}`,
      );
    }
...
${
  results.filter(
    (r) =>
      r.twoCalls.datesCount > r.singleCall.datesCount ||
      r.twoCalls.timeSlotsCount > r.singleCall.timeSlotsCount,
  ).length > 0
