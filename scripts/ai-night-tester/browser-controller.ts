...
export class BrowserController {
  ...
  private setupErrorListeners(): void {
    ...
    this.page.on("response", (response) => {
      if (response.status() >= 400) {
        ...
        if (!url.includes(".map") && !url.includes("favicon") && !url.includes("analytics")) {
  ...
  async executeAction(action: TestAction): Promise<boolean> {
    console.log(
      `🎯 Executing: ${action.type} - ${action.description || action.selector || action.url}`,
    );
        ...
        const designChecks = [
          {
            ...
            name: "Buttons",
            props: ["backgroundColor", "borderRadius", "color", "fontSize"],
          },
          {
            selector: "h2",
            name: "Headings (H2)",
            props: ["fontFamily", "fontSize", "color", "fontWeight"],
          },
          {
            selector: "a:not(:has(img))",
            name: "Links",
            props: ["color", "textDecorationLine", "fontSize"],
          },
          {
            selector: "nav a",
            name: "Nav Links",
            props: ["color", "padding", "fontSize", "fontWeight"],
          },
        ];
  ...
  private async getInteractiveElements(): Promise<InteractiveElement[]> {
    ...
    const selector =
      'button:visible, input:not([type="hidden"]):visible, textarea:visible, a:visible, [role="button"]:visible, [role="gridcell"]:visible, [role="link"]:visible, [contenteditable="true"]:visible';
    ...
    for (let i = 0; i < Math.min(locators.length, 100); i++) {
      ...
      try {
        ...
        const isInput =
          tagName === "input" ||
          tagName === "textarea" ||
          (await el.evaluate((node) => (node as HTMLElement).isContentEditable));
        ...
        if (isInput) {
          ...
          text =
            ...
            ariaLabel ||
            labelText ||
            (tagName === "textarea" ? "Zone de texte" : "Champ de saisie");
        } else {
        ...
        const detection = await el
          .evaluate((node) => {
            const style = window.getComputedStyle(node);
            const isNativeDisabled =
              (node as any).disabled === true || node.getAttribute("disabled") !== null;
        ...
        if (text === "5" || text === "10" || text === "12" || text === "27") {
          console.log(
            `🔍 [Discovery] Date "${text}": isDisabled=${detection.isDisabled}`,
            (detection as any).reasons,
          );
        }
