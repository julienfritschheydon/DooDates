...
export class ReportGenerator {
  ...
  init(): void {
    ...
    this.reportPath = path.join(config.output.reportsDir, `report-${timestamp}.md`);
...
${Array.from(this.stats.pagesVisited)
  .map((url) => `- ${url}`)
  .join("\n")}
...
`;
    ...
    if (this.reportPath) {
      ...
      fs.writeFileSync(
        this.reportPath,
        header + (this.issues.length === 0 ? "✅ No issues found!\n" : "") + issuesContent,
      );
    }

    console.log(`\n✅ Report generated: ${this.reportPath}`);
    console.log(
      `   📊 ${this.stats.totalActions} actions, ${this.stats.pagesVisited.size} pages, ${this.issues.length} issues`,
    );
