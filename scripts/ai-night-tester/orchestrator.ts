...
export class Orchestrator {
  ...
  async start(): Promise<void> {
    ...
    console.log(
      `🧠 Navigation Memory: ${memStats.totalElements} elements, ${memStats.uniquePages} pages`,
    );
  ...
  private async handleTestResult(testResult: any, context: any): Promise<void> {
    ...
    while (this.isRunning && !this.isTimeUp()) {
      try {
        ...
        if (this.isChaosMode) {
          console.log(
            `🔥 [Worker-${this.workerId}] CHAOS MODE ACTIVATED (Visit count: ${visitCount})`,
          );
        }
        ...
        if (this.mode === "bug-hunting") {
          if (!this.analysisPromise) {
            ...
            const shouldDeepAnalyze =
              this.currentPageActions === 1 || this.recentActions.length % 5 === 0;
        ...
        pageState.interactiveElements = pageState.interactiveElements.filter((el) => {
          ...
          if (el.type === "link") {
            ...
            const lastUrl =
              this.recentActions.length > 0
                ? this.recentActions[this.recentActions.length - 1].url
                : null;
            if (
              (lastUrl && el.text.toLowerCase().includes("retour")) ||
          ...
          const totalPriority =
            novelty * 0.4 + semanticWeight * 0.4 + visualWeight * 0.2 - stateChangePenalty;
          priorities.set(el.selector, totalPriority);
          return true;
        });

        // Sort elements by priority for the prompt
        pageState.interactiveElements.sort(
          (a, b) => (priorities.get(b.selector) || 0) - (priorities.get(a.selector) || 0),
        );

        // 5. Ask Gemma for next action with novelty hints
        let currentObjective =
          this.mode === "feature-discovery"
        ...
        for (const action of decision.actions) {
          ...
          if (action.type === "click" && action.selector) {
            const element = pageState.interactiveElements.find(
        ...
        if (this.browser.blockedFileUploads.length > 0) {
          const lastBlocked =
            this.browser.blockedFileUploads[this.browser.blockedFileUploads.length - 1];
          if (new Date().getTime() - lastBlocked.getTime() < 2000) {
            console.log(
              "🚫 Last action triggered a blocked file upload. Adding to exclusion list.",
            );
  ...
  private async checkForIssues(
    ...
    const shouldAnalyze =
      forceDeepAnalyze ?? (this.currentPageActions === 1 || this.recentActions.length % 5 === 0);
  ...
  private async navigateToNewPage(): Promise<void> {
    ...
    const excludedPaths = ["/date-polls/workspace"];
    ...
    if (unvisited.length > 0) {
      ...
      const performantRoutes = productRoutes.filter(
        (r) => !excludedPaths.some((p) => r.startsWith(p)),
      );
      ...
      if (performantRoutes.length > 0) {
        ...
        console.warn(
          `⚠️ No valid routes found for product ${currentProduct?.name}. Force navigating to home.`,
        );
        await this.forceNavigate();
      }
    }
  ...
  private async forceNavigate(): Promise<void> {
    const startPage =
      config.app.startPages[Math.floor(Math.random() * config.app.startPages.length)];
  ...
  private detectFrustration(action: TestAction): void {
    ...
    const isRageClick =
      last3.length >= 3 && last3.every((a) => a.type === "click" && a.selector === action.selector);

    if (isRageClick) {
      console.warn(
        `🤬 Rage Click détecté sur: ${action.selector}. Ajout à la liste noire temporaire.`,
      );
    ...
    if (last4.length === 4) {
      ...
      if (
        a1.selector &&
        b1.selector &&
        a1.selector === a2.selector &&
        b1.selector === b2.selector &&
        a1.selector === action.selector
      ) {
