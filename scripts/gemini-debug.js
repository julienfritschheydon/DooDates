...
`);
    ...
    highlight('Jours de la semaine', parsed.dayOfWeek.length > 0 ? parsed.dayOfWeek.join(', ') : 'Aucun');
    highlight('Mots-clés détectés', parsed.detectedKeywords.join(', ') || 'Aucun');
    highlight('Dates autorisées', `${parsed.allowedDates[0]} → ${parsed.allowedDates[parsed.allowedDates.length - 1]}`);
    ...
    try {
        ...
        if (postProcessed.error) {
            ...
            highlight('Dates', `${postProcessed.processing.originalDates} → ${postProcessed.processing.finalDates}`);
            highlight('Créneaux', `${postProcessed.processing.originalSlots} → ${postProcessed.processing.finalSlots}`);
            ...
            if (parsed.isMealContext && slotsCount > 1) {
                error(`PROBLÈME DÉTECTÉ: ${slotsCount} créneaux générés alors que contexte repas = 1 créneau`);
