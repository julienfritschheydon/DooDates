...
const RULES = {
  NO_WAIT_FOR_TIMEOUT: {
    ...
    message: 'Utilisez waitForElementReady, waitForReactStable ou autres helpers d\'attente conditionnelle',
    severity: 'error',
  },
  NO_SILENT_CATCH: {
  ...
  function walkDir(currentDir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        const isExcluded = normalizedRelativePath.includes('helpers/') || 
...
function checkFile(filePath, rule) {
  ...
  if (normalizedPath.includes('helpers/') || 
  ...
  for (const pattern of patterns) {
    ...
    for (const match of matches) {
      ...
      if (rule.name.includes('.catch()')) {
        ...
        if (/import.*from|type\s+\w+|interface\s+\w+|const\s+\{[^}]*error[^}]*\}/.test(fullContext)) {
          continue;
        }
        ...
        const hasPromiseBefore = /await|\.then|\.finally|Promise\.|new Promise|\.click|\.fill|\.isVisible|\.waitFor|\.evaluate|\.goto|expect\(|\.reload|\.navigate/.test(beforeMatch);
        if (!hasPromiseBefore) {
  ...
  try {
    ...
    return output
      ...
      .filter(line => {
        ...
        const isTestFile = line.includes('tests/e2e') && (line.endsWith('.ts') || line.endsWith('.js'));
        // Exclure les helpers, config, fixtures et dossiers d'anciens fichiers
        const normalizedLine = line.replace(/\\/g, '/');
        const isExcluded = normalizedLine.includes('helpers/') || 
...
function main() {
  ...
  for (const [ruleKey, rule] of Object.entries(RULES)) {
    ...
    if (ruleViolations.length > 0) {
      ...
      console.log(`${icon} ${rule.name} (${rule.severity === 'error' ? 'BLOQUANT' : 'Avertissement'})`);
      ...
      if (ruleViolations.length > maxViolationsToShow) {
        console.log(`   ... et ${ruleViolations.length - maxViolationsToShow} autre(s) violation(s)\n`);
      }
