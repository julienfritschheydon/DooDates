...
function generateReport(stdout, stderr) {
  ...
  console.log(
    `🎯 Objectif: 70% - ${stats.summary.estimatedScore >= 70 ? "✅ ATTEINT" : "❌ NON ATTEINT"}`,
  );
  console.log(`\n📄 Rapports générés:`);
...
function analyzeResults(stdout, stderr) {
  ...
  // Compter les succès
  const successMatches = stdout.match(
    /Date Poll successfully generated|Form Poll successfully generated/g,
  );
...
function analyzeByCategory(stdout) {
  ...
  const successLines = stdout.split("\n").filter((line) => line.includes("successfully generated"));
...
function generateRecommendations(stats) {
  ...
  if (stats.translations.patterns["la semaine non traduit"] > 0) {
    recommendations.push({
      ...
      action:
        'Ajouter la traduction de "la semaine" → "the week" ou mieux gérer "la semaine prochaine"',
    });
  }
