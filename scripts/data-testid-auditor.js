 ...
 * 
 * Fonctionnalités :
 * - Détecte les boutons sans data-testid
 * - Vérifie la cohérence des noms
 * - Génère des suggestions de data-testid
 * - Peut corriger automatiquement certains cas
 * 
 * Usage : node scripts/data-testid-auditor.js [options]
 */
...
// Patterns pour extraire le texte des boutons
const TEXT_PATTERNS = [
  />([^<]+)</,
  /children={([^}]+)}/
];
...
class DataTestIdAuditor {
  ...
  generateSuggestedTestId(buttonHtml, line, filePath) {
    ...
    // Extraire des attributs utiles
    const onClickMatch = buttonHtml.match(/onClick={([^}]+)}/);
    const variantMatch = buttonHtml.match(/variant="([^"]+)"/);
    const sizeMatch = buttonHtml.match(/size="([^"]+)"/);
    
    // Générer un data-testid basé sur le contexte
    ...
    // Si pas de texte, utiliser des heuristiques
    if (onClickMatch) {
      const onClick = onClickMatch[1];
      if (onClick.includes('navigate')) return `${componentName}-navigate`;
      if (onClick.includes('submit')) return `${componentName}-submit`;
      if (onClick.includes('delete')) return `${componentName}-delete`;
      if (onClick.includes('edit')) return `${componentName}-edit`;
      if (onClick.includes('close')) return `${componentName}-close`;
    }
    
    if (variantMatch) {
      const variant = variantMatch[1];
      if (variant === 'destructive') return `${componentName}-delete`;
      if (variant === 'outline') return `${componentName}-secondary`;
    }
      ...
      try {
        ...
        issues.forEach(issue => {
          ...
          if (fixedLine !== targetLine) {
            ...
            if (this.options.verbose) {
              console.log(`✅ ${filePath}:${issue.line} - Ajouté data-testid="${issue.suggestedTestId}"`);
            }
          }
        });
  ...
  generateRecommendations() {
    ...
    if (this.stats.missingDataTestId > this.stats.totalButtons * 0.1) {
      recommendations.push({
        type: 'coverage',
        priority: 'medium',
        message: 'Taux de couverture data-testid inférieur à 90%',
        action: 'Considérer une revue complète des composants'
      });
    }
  ...
  displaySummary() {
    ...
    const coverage = ((this.stats.totalButtons - this.stats.missingDataTestId) / this.stats.totalButtons * 100).toFixed(1);
...
Exemples:
  node scripts/data-testid-auditor.js
  node scripts/data-testid-auditor.js --fix --verbose
  node scripts/data-testid-auditor.js --output ./custom-reports
        `);
        process.exit(0);
    }
  }
