...
function addErrorHandlingImport(filePath) {
  try {
    ...
    if (content.includes('@/lib/error-handling') || 
        content.includes('@/hooks/useErrorHandling')) {
      console.log(`✓ ${path.relative(process.cwd(), filePath)} - already has error handling import`);
      return false;
    }
