...
const EXPECTED_EXPORTS = {
  'date-polls': ['getPolls', 'addPoll', 'deletePollById', 'duplicatePoll', 'getPollBySlugOrId', 'savePolls', 'validatePoll', 'isDatePoll'],
  'form-polls': ['getPolls', 'addPoll', 'deletePollById', 'duplicatePoll', 'getPollBySlugOrId', 'savePolls', 'validatePoll', 'isFormPoll', 'addFormResponse', 'getFormResponses', 'getFormResults'],
  'quizz': ['getPolls', 'addPoll', 'deletePollById', 'duplicatePoll', 'getPollBySlugOrId', 'savePolls', 'validatePoll']
};
  ...
  for (const [product, expectedFunctions] of Object.entries(EXPECTED_EXPORTS)) {
    ...
    for (const func of expectedFunctions) {
      ...
      const hasExport = content.includes(`export`) && (
        ...
        content.includes(`get${product.split('-').map(w => w[0].toUpperCase() + w.slice(1)).join('')}${func.replace('get', '').replace('Poll', '')}`)
      );
...
function checkImportsFromPollStorage() {
  ...
  for (const file of files) {
    ...
    if (matches) {
      ...
      const usesOldFunctions = content.includes('getPolls') || 
                               content.includes('addPoll') || 
                               content.includes('deletePollById');
      
      if (usesOldFunctions && !relativePath.includes('pollStorage.ts') && !relativePath.includes('products')) {
  ...
  if (problematicImports.length > 0) {
    warnings.push(`⚠️  Fichiers utilisant pollStorage (pourrait bénéficier d'une migration vers products/):`);
