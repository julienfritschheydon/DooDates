...
const testCases = [
    ...
    { input: "Crée un sondage pour un week-end jeux. Ajoute tous les samedis de mars 2026", expected: "Create a poll for a games weekend. Add every saturday in march 2026" },
];
...
testCases.forEach((testCase, index) => {
    ...
    try {
        ...
        console.log(`\n   📊 Résultats:`);
        console.log(`      Chrono FR: ${parsedFr.length > 0 ? `✅ "${parsedFr[0].text}" → ${parsedFr[0].start.date().toISOString().split('T')[0]}` : "❌ RIEN"}`);
        console.log(`      Chrono EN: ${parsedEn.length > 0 ? `✅ "${parsedEn[0].text}" → ${parsedEn[0].start.date().toISOString().split('T')[0]}` : "❌ RIEN"}`);
        
        const improvement = parsedFr.length === 0 && parsedEn.length > 0;
        console.log(`      ${improvement ? "🎯 AMÉLIORATION !" : parsedEn.length > 0 ? "✅ OK" : "❌ Échec"}`);
...
if (improvementCount > 0) {
    ...
    results.filter(r => r.improvement).forEach((result, index) => {
